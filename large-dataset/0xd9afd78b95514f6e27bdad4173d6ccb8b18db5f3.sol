...
contract Converter is ERC20, Ownable {

    using SafeMath for uint256;

 

    uint256 public dollarFeeRatio   = 200;  // USDC/ESD fee ratio

    uint256 public treasuryFeeRatio = 2000; // fee ratio of treasury

    uint256 public baseFactor       = 10000;// ratio base
    ...
    constructor(address _dollar, address _treasuryAddr, uint256 _dollarDecimal) 
        ...
        public  ERC20("Dynamic USDC Token","dUSDC")

    {
        ...
        dollarDecimal   = _dollarDecimal;

    }



    function reset( uint256 _dollarFeeRatio, uint256 _treasuryFeeRatio, 
...

    {
        ...
        dollarFeeRatio = _dollarFeeRatio;
    ...
    function applyFee(uint256 _feeInBips, uint256 _amount) internal view returns (uint256) {
        ...
        return _amount.mul(baseFactor.sub(_feeInBips)).div(baseFactor); 
    ...
    function calculateFee(uint256 _feeInBips, uint256 _amount) internal view returns (uint256) {
        ...
        return _amount.mul(_feeInBips).div(baseFactor); 
    ...
    function convert(uint256 _amount) external {
        ...
        // usdc/esd fee, portion of it to treasury

        // the remaining will be rewarded to the former partners

        dollar.transferFrom(msg.sender, address(this), _amount);



        // mint shares with total fee
        ...
        // transfer the specified amount to treasury.

        uint256 dollarFee = _amount.mul(dollarFeeRatio).div(baseFactor);

        uint256 dollarFeeForTreasury = dollarFee.mul(treasuryFeeRatio).div(baseFactor);



        // transfer the specified dollarFee to treasury.

        dollar.transfer(treasuryAddr, dollarFeeForTreasury);





        // check if get into the hardCap, need check the stake balance

        // if(dollar.balanceOf(address(this)) >= dollarHardCap) {

        //     uint256 stakingBalance = staking.getStakedBalance();

        //     require(stakingBalance >= dfiStakingNum, "staking power is not enough.");
    ...
    function redeem(uint256 _value) external {
        ...
        require(balanceOf(msg.sender) >= _value && _value > 0, "not enough");
        ...
        uint256 value = _value.mul(dollar.balanceOf(address(this))).div(totalSupply());
