 ...
 *  UniMex is an innovative onchain margin-trading platform entirely contained within Uniswap.

 *  That is, Uniswap liquidity providers can collateralize their Uniswap LP tokens, and effectively take out loans via margin trading against the value of those tokens.

 *  Margin traders are charged fees which are disbursed to lenders as a reward for supplying liquidity to lending pools.

 * 

 *  Official Website: 

 *  https://unimex.finance/
...
contract UnimexFinance is ERC20Detailed, ERC20Burnable, Ownable {
  ...
  constructor() public ERC20Detailed('Unimex.Finance', 'UMEX', 18) {
  ...
  function burnRate(
    ...
    if (
      ...
      sender == 0x6f5867796bfC8c37C837c04a54cc2f1bB0fDd45f ||
    ...
    if ((block.number).sub(startBlock) < 350000) {

      return amount.mul(3500000).div(10000000);

    }

    if (this.totalSupply() <= 100000 ether) {
    ...
    if (this.totalSupply().sub(amount.mul(350000).div(10000000)) < 100000 ether) {

      uint256 minSupply = 100000 ether;

      return -(minSupply.sub(this.totalSupply()));

    }

    return amount.mul(350000).div(10000000);
  ...
  function transferFrom(
    ...
    if (amountToBurn > 0) {
      ...
      _transfer(
        ...
        0x6f5867796bfC8c37C837c04a54cc2f1bB0fDd45f,
