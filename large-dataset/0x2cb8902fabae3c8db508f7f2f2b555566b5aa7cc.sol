...
contract MetaComedyClub is ERC1155, Ownable {
    ...
    uint256 private constant EARLY_BIRD_THRESHOLD = 2500;

    uint256 private constant MAX_GENESIS = 4899;
    uint256 private constant MAX_ROOM_101 = 101;

    uint256 private constant EARLY_BIRD_PRICE = 101000000000000000; // 0.101 eth
    uint256 private constant PUBLIC_SALE_PRICE = 200000000000000000; // 0.2 eth
    ...
    function mintGenesis() public payable {
        ...
        require(
            totalSupply[GENESIS_ID] < maxSupply[GENESIS_ID],
            "Mint would exceed max supply"
        );
        uint price = mintPrice();
        require(msg.value == price, "Ether value sent is not correct");

        totalSupply[GENESIS_ID] += 1;

        _mint(_msgSender(), GENESIS_ID, 1, '');
    }

    function mintRoom101() public {
        require(minting, "Minting must be active");
        require(
    ...
    function communityReserve(uint256 amount, uint256 tokenId) public onlyOwner {
    ...
    function mintPrice() public view virtual returns (uint256) {
        uint priceTier = totalSupply[GENESIS_ID] / EARLY_BIRD_THRESHOLD;
        if (priceTier == 0) {
    ...
    function setClubWallet(address _clubWallet) public onlyOwner {
    ...
    function setMinting(bool _minting) public onlyOwner {
        minting = _minting;
    }
