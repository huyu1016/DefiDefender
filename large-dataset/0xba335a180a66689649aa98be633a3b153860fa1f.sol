...
contract preSale is owned {
    ...
    uint256 private runPeriod = 0 days;



    /// the intical price of token

    uint256 private tokenPerETH = 0;



    /// the max token can sold 50 K + 25 K

    uint256 private hardCap = 0;
    ...
    function initialize(address _token, address payable _beneficiary, uint256 _startsAt, uint256 _runPeriod, uint256 _tokenPerETH, uint256 _hardCap) public returns (bool) {
        ...
        token = stackXfinance(_token);

        hardCap = uint256(_hardCap).mul( 10 ** uint256(token.decimals()));

        beneficiary = _beneficiary;

        // Mint tokens for PreSale

        token.mint(address(this), hardCap);
    ...
    function _buy(address payable _receiver) internal {

        require(getEndTime() > 0 , "Time Out");

        require(uint(msg.value).mul(tokenPerETH).add(tokensSold) < hardCap , "HardCap reached");



        uint256 tokensAmount = uint(msg.value).mul(tokenPerETH);
    ...
    function burnRestToken() public returns (bool) {
        ...
        require(uint(startsAt).add(runPeriod) < now, "PreSale is not end yet.");
