...
contract nft is ERC721, Ownable {
    ...
    uint private constant mintCostPresale = 0.05 ether;
    uint private constant mintCostPublicSale = 0.055 ether;
    
    //maximum amount of mints allowed per person
    uint256 public constant maxMintPresale = 4;
    uint private constant maxMintPublicSale = 6;
    ...
    uint private maxReservedMints = 55;
    ...
    constructor() ERC721('Duckfi', 'DF') {
        numberOfTotalTokens = 0;
        maxTotalTokens = 5555;

        unrevealedURI = "ipfs://QmTCZEQEXPUHESwwTWboq5pum62oUbkVpZU5mhg8dg1AgX";
    }
    ...
    function switchToPresale() public onlyOwner {
        ...
        publicSaleLaunchTime = presaleLaunchTime + 14400;
        revealTime = publicSaleLaunchTime + 43200;
    }
    ...
    */
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(msg.value >= mintCost() * number, "Not sufficient Ether to mint this amount of NFTs (Cost = 0.05 ether each NFT)");
    ...
    function publicSaleMint(uint256 number) public payable {
        ...
        require(numberOfTotalTokens + number <= maxTotalTokens - (maxReservedMints - _reservedMints), "Not enough NFTs left to mint..");
        require(number <= maxMint(), "Maximum 6 Mints per Tx allowed!");
        require(msg.value >= mintCost() * number, "Not sufficient Ether to mint this amount of NFTs (Cost = 0.055 ether for each NFT)");
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(payable(0x5cD7e73A9fbB2eDA797d0509d218Dae5f218ffd6), balance);
    }
    ...
    //get the max amount allowed to mint
    function maxMint() public view returns(uint) {
        State saleState_ = saleState();
        if (saleState_ == State.Presale || saleState_ == State.NoSale) {
            return maxMintPresale;
        }
        else {
            return maxMintPublicSale;
        }
    }
