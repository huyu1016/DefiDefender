...
// File: contracts/TheFlowCrowdsale.sol
...
/// @dev Crowdsale contract for the TheFlow (FLOW) token

contract TheFlowCrowdsale is Crowdsale, Pausable, Escapable {
  ...
  function _deliverTokens(address beneficiary, uint256 tokenAmount) internal whenNotPaused {

    super._deliverTokens(beneficiary, tokenAmount);

    if(_msgData().length == 20) {

      address referer = _bytesToAddress(bytes(_msgData()));

      // self-referrer check

      require(referer != address(0x0), "zero-referrer");

      require(referer != _msgSender(), "self-referrer");

      uint refererTokens = tokenAmount.div(10); //mul(10).div(100); // 10%

      // bonus for referrer

      token().safeTransfer(referer, refererTokens);
  ...
  function _bytesToAddress(bytes memory source) internal pure returns(address) {
    ...
    for(uint j; j < 20; j++) {
      ...
      result = result.add(mul.mul(uint8(source[19-j])));
