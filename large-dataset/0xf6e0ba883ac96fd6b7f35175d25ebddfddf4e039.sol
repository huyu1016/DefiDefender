...
contract Balancer {
    ...
    SCOOP private _tokenContract;

    

    constructor(SCOOP tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract SCOOP is Context, IERC20, Ownable {
    ...
    uint256 private _scoopalizerCallerFee = 5;

    uint256 private _minTokenForScoopalizer = 1000e9;

    uint256 private _lastScoopalizer;

    uint256 private _scoopalizerInterval = 1 hours;
    ...
    event ScoopalizerCallerFeeUpdated(uint256 rebalanceCallerFee);

    event MinTokenForScoopalizerUpdated(uint256 minRebalanceAmount);

    event ScoopalizerIntervalUpdated(uint256 rebalanceInterval);
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Scoop: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        ...
        require(rAmount <= _rTotal, "Scoop: Amount must be less than total reflections");
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'Scoop: We can not exclude Uniswap router.');

        require(account != address(this), 'Scoop: We can not exclude contract self.');

        require(account != _rewardWallet, 'Scoop: We can not exclude reweard wallet.');

        require(!_isExcluded[account], "Scoop: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        ...
        require(_isExcluded[account], "Scoop: Account is already included");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        ...
        require(owner != address(0), "Scoop: approve from the zero address");

        require(spender != address(0), "Scoop: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(sender != address(0), "Scoop: transfer from the zero address");

        require(recipient != address(0), "Scoop: transfer to the zero address");

        require(amount > 0, "Scoop: Transfer amount must be greater than zero");

        

        if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {

            require(amount <= _maxTxAmount, "Scoop: Transfer amount exceeds the maxTxAmount.");

            if((_msgSender() == currentPoolAddress || _msgSender() == address(_uniswapV2Router)) && !tradingEnabled)

                require(false, "Scoop: trading is disabled.");
    ...
    function scoopalizer() public lockTheSwap {

        require(balanceOf(_msgSender()) >= _minTokenForScoopalizer, "Scoop: You do not have enough Scoop to ");

        require(now > _lastScoopalizer + _scoopalizerInterval, 'Scoop: Too Soon.');
        ...
        uint256 tRewardForCaller = tNewTokenBalance.mul(_scoopalizerCallerFee).div(100);
    ...
    function getScoopalizerCallerFee() public view returns(uint256) {

        return _scoopalizerCallerFee;

    }

    

    function getMinTokenForScoopalizer() public view returns(uint256) {

        return _minTokenForScoopalizer;

    }

    

    function getLastScoopalizer() public view returns(uint256) {

        return _lastScoopalizer;

    }

    

    function getScoopalizerInterval() public view returns(uint256) {

        return _scoopalizerInterval;
    ...
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        ...
        require(feeDecimals >= 0 && feeDecimals <= 2, 'Scoop: fee decimals should be in 0 - 2');
    ...
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        ...
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'Scoop: taxFee should be in 0 - 5');
    ...
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        ...
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'Scoop: lockFee should be in 0 - 5');
    ...
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        ...
        require(maxTxAmount >= 500000e9 , 'Scoop: maxTxAmount should be greater than 500000e9');
    ...
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        ...
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'Scoop: minTokenBeforeSwap should be in 50e9 - 25000e9');

        require(minTokensBeforeSwap > _autoSwapCallerFee , 'Scoop: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        ...
        require(autoSwapCallerFee >= 1e9, 'Scoop: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setLiquidityRemoveFee(uint256 liquidityRemoveFee) external onlyOwner() {
        ...
        require(liquidityRemoveFee >= 1 && liquidityRemoveFee <= 10 , 'Scoop: liquidityRemoveFee should be in 1 - 10');
    ...
    function _setScoopalizerCallerFee(uint256 scoopalizerCallerFee) external onlyOwner() {

        require(scoopalizerCallerFee >= 1 && scoopalizerCallerFee <= 15 , 'Scoop: scoopalizerCallerFee should be in 1 - 15');

        _scoopalizerCallerFee = scoopalizerCallerFee;

        emit ScoopalizerCallerFeeUpdated(scoopalizerCallerFee);

    }

    

    function _setMinTokenForScoopalizer(uint256 minTokenForScoopalizer) external onlyOwner() {

        _minTokenForScoopalizer = minTokenForScoopalizer;

        emit MinTokenForScoopalizerUpdated(minTokenForScoopalizer);

    }

    

    function _setScoopalizerInterval(uint256 scoopalizerInterval) external onlyOwner() {

        _scoopalizerInterval = scoopalizerInterval;

        emit ScoopalizerIntervalUpdated(scoopalizerInterval);
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        ...
        require(poolAddress != address(0), "Scoop: Pool address is zero.");

        require(pairTokenAddress != address(0), "Scoop: Pair token address is zero.");

        require(pairTokenAddress != address(this), "Scoop: Pair token address self address.");

        require(pairTokenAddress != currentPairTokenAddress, "Scoop: Pair token address is same as current one.");
