...
// File: contracts/artifacts/Badass Ninjas.sol
...
contract BadassNinjas is ERC721Enumerable, Ownable {
  ...
  uint256 public Price = 0.09 ether; // Main Price
  uint256 public PricePS = 0.07 ether;
  uint256 public MaxCollection = 7777;

  constructor(string memory _initBaseURI,string memory _initNotRevealedUri)
    ERC721("Badass Ninjas", "BAN") 
    { setBaseURI(_initBaseURI); setNotRevealedURI(_initNotRevealedUri); }
  ...
  function mint(uint256 _mintAmount) public payable {
   ...
   if (msg.sender != owner()) {
        if (isPresalesMode == true) {
            ...
            require(ownerMintedCount + _mintAmount <= 5, "PS: Max NFT per Wallet Reached");
            require(supply + _mintAmount <= 3000, "PS: No more NFTs available");
            require(msg.value >= PricePS * _mintAmount, "PS: Balance Insufficient");
        }
        else {
            require(!isPresalesMode, "");
            uint256 ownerMintedCount = WB[msg.sender];
            require(ownerMintedCount + _mintAmount <= 10, "MS: Max NFT per Wallet Reached");
            require(msg.value >= Price * _mintAmount, "Main Sale: Balance Insufficient"); 
        }
    }
  ...
  function addWW(address[] calldata _users) public onlyOwner {
    delete WW;
    WW = _users;
  }
...
function setPricePS(uint256 _newPricePS) public onlyOwner {
  ...
  function withdraw() public payable onlyOwner {
    (bool bn, ) = payable(owner()).call{value: address(this).balance}("");
    require(bn);
  }
}
