...
// File: contracts/BZxLoanEtherSwap.sol
...
contract BZxLoanEtherSwap is RateNormalization, NetworkBasedTokenSwap

{
...

  {
    ...
    uint rateStored = ieth.tokenPrice();

    uint rate = 0;

    if (src == ETHER && dest == address(ieth)) {

      rate = expScaleSquare.div(rateStored);

    } else if (src == address(ieth) && dest == ETHER) {
...

  {
    ...
    if (src == ETHER && dest == address(ieth)) {
      ...
      return ieth.mintWithEther.value(srcAmount)(address(this));

    } else if (src == address(ieth) && dest == ETHER) {

      return ieth.burnToEther(address(this), srcAmount);
