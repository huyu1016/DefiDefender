...
contract WalkingApesPastel is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public presaleMintMax = 10;
  mapping(address => uint8) public presaleMintCount;

  uint256 public clearanceMintMax = 2; 
  ...
  //sale holders
  address[4] public fundRecipients = [
        0x450F7acBA06ae034964e29d9c078AaEABd773347,
        0xcCbfa0bEE688074922de3d40a235Ba1b97b1D29F,
        0x849CA417cA4619450F874A84C3D5936AA34C7266,
        0x4Fd96519eceC2858946606818a398c364A018911
  ];
  uint256[] public receivePercentagePt = [4700, 2800, 2000, 500];   
  ...
  constructor(
    ...
    setHiddenMetadataUri("ipfs://QmNhxJbxkgydjWHgufQa98CrPpoFfWaJzck8UuUG2oj4rr/wap-metadata.json");
  }
  ...
  // public
  function freemint(uint8 _mintAmount) public payable {
  ...
  function salemint(uint8 _mintAmount) public payable {
    uint256 supply = totalSupply();
    uint256 mintAmount = _mintAmount;
    require(!paused);
    require(_mintAmount > 0);
  ...
  function Mint(uint256 _mintAmount, address _to) public onlyOwner {
    uint256 supply = totalSupply();
    uint256 mintAmount = _mintAmount;
    for (uint256 i = 1; i <= mintAmount; i++) {
        if(!_exists(supply + i)){
