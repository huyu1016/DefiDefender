...
contract BorgPresale is Context, ReentrancyGuard {
    ...
    address payable constant owner1 = 0xDe87EA52cD67a32eC71d1A9817856f532b3145bf; // BORG Marketing

    address payable constant owner2 = 0x635bF673DB15bd80846ed9eD0091D7B308b86D9d; // BORG Treasury

    address payable constant owner3 = 0x6fE00946Dfa366360b8BB02a68d5536d8D92d488; // BORG Development Fund
    ...
    function setBORG(IERC20 addr) external onlyOwner nonReentrant {
    ...
    function buyBORGTokens() public payable nonReentrant {
        ...
        require(msg.value >= 0.05 ether, "Must send more than 0.05 ETH");

        require(msg.value <= 5 ether, "You can only send 5 ETH Max per TX.");

        require(ethSent < 500 ether, "Hard Cap reached at 500 ETH");

        require (msg.value.add(ethSent) <= 500 ether, "Hard Cap is 500 ETH");

        require(ethSpent[msg.sender].add(msg.value) <= 20 ether, "You cannot buy more, 20 ETH total per address or 5 ETH per TX.");
    ...
    function claimTeamFeeAndAddLiquidity() external onlyOwner  {
       ...
       uint256 LowamountETH = address(this).balance.mul(1).div(100); 

       owner1.transfer(LowamountETH);

       owner2.transfer(LowamountETH);

       owner3.transfer(LowamountETH);
