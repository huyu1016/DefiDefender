...

{
    ...
    function claimComps(address dfWallet, uint256 minDAIForCompound, bytes memory data) public {
        ...
        if (minDAIForCompound > 0) {
            ...
            uint256 daiAmount = exchangeInternal(IToken(COMP_ADDRESS), compTokenBalance, IToken(DAI_ADDRESS), minDAIForCompound, data);

            if (fee > 0) daiAmount = daiAmount * sub(100, fee) / 100;

        

            IToken(DAI_ADDRESS).transfer(msg.sender, daiAmount);

            wallets[dfWallet].compClaimedinUSD += uint64(daiAmount / 1e12); // 6 decemals

        } else {

            if (fee > 0) compTokenBalance = compTokenBalance * sub(100, fee) / 100;
    ...
    struct UserData {
        ...
        // uint256 targetAmount; // 6 decemals

        uint64 compClaimed;

        uint64 compClaimedinUSD; // 6 decemals
    ...
    function closeDepositDAI(address dfWallet, uint256 minDAIForCompound, bytes memory data) public balanceCheck {
        ...
        uint256 startBalance = ICToken(DAI_ADDRESS).balanceOf(address(this));
        ...
        IDfWallet(dfWallet).withdrawToken(DAI_ADDRESS, msg.sender, userBalance); // withdraw original dai amount
        ...
        if (minDAIForCompound > 0) {
            ...
            uint256 amountDai = exchangeInternal(IToken(COMP_ADDRESS), IToken(COMP_ADDRESS).balanceOf(dfWallet), IToken(DAI_ADDRESS), minDAIForCompound, data);

            if (fee > 0) amountDai = amountDai * sub(100, fee) / 100;

            wallets[dfWallet].compClaimedinUSD += uint64(amountDai / 1e12);

            IToken(DAI_ADDRESS).transfer(msg.sender, amountDai);

            emit DfCloseDeposit(dfWallet, amountDai, DAI_ADDRESS);
    ...
    // TODO: remove uint256 coef

    function createStrategyDeposit(uint256 amountDAI, uint256 flashLoanAmount, uint256 extractAmount) public balanceCheck {
        ...
        IDfWallet(dfWallet).deposit(DAI_ADDRESS, CDAI_ADDRESS, totalFunds, DAI_ADDRESS, CDAI_ADDRESS, extractAmount);
