// @title:  333 SUPER POSITIVE WORDS
// @desc:   333 SUPER POSITIVE, UPLIFTING AND MOTIVATING WORDS TO MAKE YOUR DAY
...
contract SuperPositiveWords is ERC721, Ownable, ReentrancyGuard {
    ...
    EnumerableSet.AddressSet private presaleWhitelist;
    EnumerableSet.AddressSet private allowedWalletAddresses;

    
    string private baseURI;
    string private unrevealedTokenURI = "ipfs://QmXkG6znQUJcfcbXCXKUa8hvJWvbM7UEnmyZJCyCg8Zp5p/placeholder.json";
    uint256 public maxTokens = 333;
    uint256 public maxPresaleTokens = 10;
    uint256 public maxTokensPerWallet = 5;
    uint256 public maxMintPerTxPresale = 5;
    uint256 public maxMintPerTx = 5;
    uint256 public tokensReserved = 133;
    ...
    string public SPW_PROVENANCE;

    address a1 =0xFAB1AD346828CFA0Af375042498648FEEFe6A56B;
    address a2 =0x66EDC27274E448B56241459AE7387E39Ae86be1b;
    ...
    constructor(string memory _ipfsCID)
        ERC721("333 Super Positive Words", "SPW")
    {
    ...
    /*
     * Full reveal of the Super Postive Words would be 1 day from the day of deployment of the contract.
     */    
        REVEAL_TIMESTAMP = block.timestamp + 86400 * 1;
    }
    ...
    function getPresaleWhitelist() public onlyOwner view returns (address[] memory) {
        return presaleWhitelist.values();
    }
    ...
    */
        ...
        whitelistFunction(_addresses, _presale)
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            if (_presale) {
                ...
                presaleWhitelist.add(_addresses[i]);
            } else {
        ...
        whitelistFunction(_addresses, _presale)
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            if (_presale) {
                presaleWhitelist.remove(_addresses[i]);
            } else {
    ...
    function checkAddressForPresale (address _address) view public returns(bool){
      
          if(presaleWhitelist.contains(_address))      
          {
              return true;      
          }else{
              return false;
    ...
    function checkAddressForPublicSale (address _address) view public returns(bool){       

           if(whitelist.contains(_address))      
          {
              return true;      
          }else{
              return false;
      ...
      function claimReserved(uint256 _amount) public onlyOwner {
        ...
        require(_amount <= tokensReserved, "Can't claim more than reserved tokens left.");     
    ...
    */
    function mintSuperWords(uint256 _amount) public payable {
        ...
        if (presaleActive) {
            if (owner() != msg.sender) {
                require(
                    presaleWhitelist.contains(msg.sender),
        ...
        if (owner() != msg.sender) {
            require(
                maxTokens >
                    _amount + totalSupply.current() + tokensReserved + 1,
                "Not enough tokens left!"
            );
            require(
    ...
    function withdraw(uint256 _weiAmount, address _to) public onlyOwner nonReentrant {

            allowedWalletAddresses.add(a1);
            allowedWalletAddresses.add(a2);

           require(allowedWalletAddresses.contains(_to),"This address is not in allowed list"); 
