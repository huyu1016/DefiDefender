...
contract MetaGoats is ERC721Enumerable, Ownable {
    using Strings for uint256;

    string public baseURI;
    string public baseExtension = ".json";
    string public notRevealedUri;
    uint256 public presaleCost = 0.0666 ether;
    uint256 public vipCost = 0.1 ether;
    ...
    // public
    function vipMint(uint256 _mintAmount) public payable {
        require(!paused);
        uint256 supply = totalSupply();
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount);
    ...
    function addWhitelistUser(address _users) public payable {
        whitelistedAddresses.push(_users);
    }

    function addVipUser(address _users) public payable {
        vipAddresses.push(_users);
    }

    function totalVipUsers() public view onlyOwner returns (uint256) {
        return vipAddresses.length;
    }

    function totalWhitelistUsers() public view onlyOwner returns (uint256) {
