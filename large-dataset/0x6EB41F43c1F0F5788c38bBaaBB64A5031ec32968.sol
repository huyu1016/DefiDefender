...
// File: contracts/Unfazed Feline.sol
...
contract UnfazedFeline is ERC721Enumerable, Ownable {
  ...
  constructor(string memory _initBaseURI)
    ERC721("Unfazed Feline", "UF") { setBaseURI(_initBaseURI); }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(supply + _mintAmount <= MaxToken, "Sold Out");   
     if (msg.sender != owner()) {
        require(supply + _mintAmount <= 9975, "Public Sold Out");       
        uint256 ownerMintedCount = WB[msg.sender];
        require(ownerMintedCount + _mintAmount <= 15, "MS: Max NFTs Reached");
