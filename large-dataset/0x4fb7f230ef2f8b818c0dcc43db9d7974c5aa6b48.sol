...
contract $4RFIPresale is Context, ReentrancyGuard {
    ...
    uint totalNumberForPresale = 4800; 

    uint maxEthHardCap = 80;
    ...
    function buyTokens() public payable nonReentrant {
        ...
        //require(msg.value <= 2.1 ether, "You sent more than 2 ETH");

        require(msg.value <= 2 ether, "You sent more than 2 ETH");



        //4RFI: 80 ETH presale cap

        require(ethSent < 80 ether, "Hard cap reached");

        require (msg.value.add(ethSent) <= 80 ether, "Hardcap reached");



        //4RFI: 2 ETH limit

        require(ethSpent[msg.sender].add(msg.value) <= 2 ether, "You cannot buy more than 2 ETH");
