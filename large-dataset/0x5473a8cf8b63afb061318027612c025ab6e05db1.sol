...
contract BADGER is Ownable, ERC721 {
    ...
    constructor() ERC721("Crypto Badger Clan", "CBC"){
        pre_one_package=0.08 ether;
        pre_two_package=0.16 ether;
        pre_three_package=0.4 ether;

        one_package=0.08 ether;
        two_package=0.16 ether;
        three_package=0.4 ether;

        presale_total=1000;
    }
   function update_presale_price(uint one, uint two, uint three, uint total)external onlyOwner{
        pre_one_package=one;
        pre_two_package=two;
        pre_three_package=three;
        presale_total= total;
    }
      function update_public_price(uint one, uint two, uint three)external onlyOwner{
        one_package=one;
        two_package=two;
        three_package=three;
    }
   function buy(uint _count) public payable{
       ...
       uint quantity=0;
        ...
        if(msg.value == three_package){quantity=5;}
   ...
   function buy_presale(uint _count) public payable{
         ...
         require(checkbuyer() + _count <= _walletLimit, "Wallet limit Reached");
         require(_count == 1 || _count == 2 || _count == 5, "Incorrect amount");
         require(totalSupply <= presale_total,"Presale Tokens Sold out");
        uint quantity=0;
        if(msg.value == pre_one_package){quantity=1;}
        if(msg.value == pre_two_package){quantity=2;}
        if(msg.value == pre_three_package){quantity=5;}
           for(uint i = 0; i < quantity; i++)
           { _safeMint(msg.sender, totalSupply + 1);
               totalSupply += 1; 
               addbuyer();
