...
contract PhotonX is ERC20 {
    ...
    string public constant name = "PhotonX network";

    string public constant symbol = "Phtx";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 2800000*(10**uint256(decimals));
    ...
    uint256 public developmentTokensSent;

    uint256 public teamTokensSent;

    

    mapping(address => bool) public developmentTokenHolder;

    mapping(address => uint256) public developmentTokensReleased;

    mapping(address => uint256) public developmentTokensInitially;
    ...
    mapping (address => uint256) public lockingTimeTeam;
    ...
    * @dev this function will send the development tokens to given address
    ...
    */
    ...
    function sendDevelopmentTokens(address _userAddress, uint256 _value) external whenNotPaused onlyOwner returns (bool) {



     developmentTokenHolder[_userAddress] = true;

     developmentTokensInitially[_userAddress] = developmentTokensInitially[_userAddress].add(_value);

     lockingTime[_userAddress] = now;

     developmentTokensSent = developmentTokensSent.add(_value);
    ...
    function getCycle(address _userAddress) public view returns (uint256){
     ...
     require(developmentTokenHolder[_userAddress]);
     ...
     else if (cycle > 7776000 && cycle <= 54432000)

     {     

    

      uint256 secondsToHours = cycle.div(7776000);
    ...
    function getCycleTeam(address _userAddress) public view returns (uint256){
     ...
     else if (cycle > 2592000 && cycle <= 64800000)

     {     

    

      uint256 secondsToHours = cycle.div(2592000);
...

    {
        ...
        if(developmentTokenHolder[msg.sender]){





           if(developmentTokensReleased[msg.sender] == developmentTokensInitially[msg.sender]){
            ...
            require(now >= lockingTime[msg.sender].add(7776000),'Zero cycle is running');



            uint256 preSaleCycle = getCycle(msg.sender);

            uint256 fifteenPercent = (developmentTokensInitially[msg.sender].mul(15)).div(100);

            

            require(developmentTokensReleased[msg.sender] != developmentTokensInitially[msg.sender], 'all tokens released');

            require(developmentTokensReleased[msg.sender] != fifteenPercent.mul(preSaleCycle),'this cycle all tokens released');            

            

            if(developmentTokensReleased[msg.sender] < fifteenPercent.mul(preSaleCycle))

            {

            uint256 tokenToSend = amount;

            developmentTokensReleased[msg.sender] = tokenToSend.add(developmentTokensReleased[msg.sender]);

            require(developmentTokensReleased[msg.sender] <= developmentTokensInitially[msg.sender],'tokens released are greater then inital tokens');
        ...
        }else if (teamTokenHolder[msg.sender]) {
           ...
           if(teamTokensReleased[msg.sender] == teamTokensInitially[msg.sender]){
            ...
            uint256 preSaleCycle = getCycleTeam(msg.sender);
    ...
    function transferFrom(
        ...
        if(developmentTokenHolder[sender]){



           if(developmentTokensReleased[sender] == developmentTokensInitially[sender]){
...
{            require(now >= lockingTime[sender].add(7776000),"zero cycle is running");



            uint256 preSaleCycle = getCycle(sender);

            uint256 fifteenPercent = (developmentTokensInitially[sender].mul(15)).div(100);

            

            require(developmentTokensReleased[sender] != developmentTokensInitially[sender]);

            require(developmentTokensReleased[sender] != fifteenPercent.mul(preSaleCycle));            

            

            if(developmentTokensReleased[sender] < fifteenPercent.mul(preSaleCycle))
            ...
            developmentTokensReleased[sender] = tokenToSend.add(developmentTokensReleased[sender]);
        ...
        }else if (teamTokenHolder[sender]) {
...
{            require(now >= lockingTime[sender].add(1296000),"zero cycle is running");
            ...
            uint256 preSaleCycle = getCycleTeam(sender);
