...
contract RavenPresale is Ownable {
  ...
  uint256 public constant INVESTMENT_LIMIT_PRESALE = 1  ether; // 1.5 ETH is maximum investment limit for pre-sale

  uint256 public constant INVESTMENT_LIMIT_DEVELOPER = 1 ether; // 2.88 ETH is maximum investment limit for developer pre-sale

  uint256 public constant INVESTMENT_LIMIT_PUBLIC = 1 ether;    //0.5 ETH is maximum investment limit for public pre-sale



  uint256 public constant INVESTMENT_RATIO_PRESALE   = 28;// divided by 100 // whitelist pre-sale rate is 0.28 ETH/$RAVEN

  uint256 public constant INVESTMENT_RATIO_DEVELOPER = 18;// divided by 100 // developer pre-sale rate is 0.18 ETH/$RAVEN

  uint256 public constant INVESTMENT_RATIO_PUBLIC = 42;   // divided by 100 // public pre-sale rate is 0.42 ETH/$RAVEN (Uniswap listing price)
...
//      console.log("refundAmount: '%s'", refundAmount);
...

  {
   ...
   if (isWhitelisted(_msgSender()) && !allowPublicInvestment) {



      require(addressTotalInvestment == INVESTMENT_LIMIT_PRESALE, "investment per whitelist pre-sale address is 1 ETH.");



      amountOfTokens = msg.value.mul(450);
      ...
      require(addressTotalInvestment == INVESTMENT_LIMIT_PUBLIC, "investment for every address (once public presale has started) is 1 ETH.");
