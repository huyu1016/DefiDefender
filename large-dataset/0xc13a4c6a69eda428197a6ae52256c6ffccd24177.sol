...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    // the amount of tokens to lock for liquidity during every transfer, i.e. 100 = 1%, 50 = 2%, 40 = 2.5%, 33 ~ 3%
    ...
    uint256 public tokenBurnDivisor;

    

    uint256 public tokenBurnedAmount;
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        if (liquidityLockDivisor != 0 && feelessAddr[from] == false && feelessAddr[to] == false) {
            ...
            uint256 tokenBurnAmount = amount.div(tokenBurnDivisor);

            super._transfer(from, address(this), liquidityLockAmount);

            super._burn(from, tokenBurnAmount);

            tokenBurnedAmount += tokenBurnAmount;

            super._transfer(from, to, amount.sub(liquidityLockAmount).sub(tokenBurnAmount));
...
contract SoarsFinance is 

    ERC20(30000 ether), 

    ERC20Detailed("SoarsFinance", "SOS", 18), 
...

{
    ...
    function setUniswapV2Router(address _uniswapV2Router) public onlyWhitelistAdmin {
        ...
        require(uniswapV2Router == address(0), "SOS::setUniswapV2Router: already set");
    ...
    function setUniswapV2Pair(address _uniswapV2Pair) public onlyWhitelistAdmin {
        ...
        require(uniswapV2Pair == address(0), "SOS::setUniswapV2Pair: already set");
    ...
    function setLiquidityLockDivisor(uint256 _liquidityLockDivisor) public onlyWhitelistAdmin {
        ...
        if (_liquidityLockDivisor != 0) {
            ...
            require(_liquidityLockDivisor >= 10, "SOS::setLiquidityLockDivisor: too small");
    ...
    function setRebalanceDivisor(uint256 _rebalanceDivisor) public onlyWhitelistAdmin {
        ...
        if (_rebalanceDivisor != 0) {
            ...
            require(_rebalanceDivisor >= 10, "SOS::setRebalanceDivisor: too small");
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor) public onlyWhitelistAdmin {
        ...
        if (_rewardDivisor != 0) {
            ...
            require(_rewardDivisor >= 10, "SOS::setCallerRewardDivisor: too small");
