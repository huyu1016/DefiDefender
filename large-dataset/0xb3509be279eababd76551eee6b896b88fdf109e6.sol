...
contract Stake is StakingInterface, Ownable, TokenErrorReporter {
    ...
    function addOperatorRoles(address [] memory _operators) public onlyOwner
    ...
    function balanceOf(address staker, address tokenAddress) public view stakeExists(staker, tokenAddress) returns (uint256) {

        if(isStake(staker, tokenAddress)) {

            Types.Stake memory memStake = stakes[staker][tokenAddress];

            uint256 balance = memStake.TokenBalance;
