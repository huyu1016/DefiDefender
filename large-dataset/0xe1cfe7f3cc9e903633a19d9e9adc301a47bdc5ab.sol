...
contract ERC20 is Context, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        uint256 burntTokens = amount * 2 /100;

        _burn(msg.sender, burntTokens);       

        uint256 leftTokens = amount - burntTokens;

        _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");

        _balances[recipient] = _balances[recipient].add(leftTokens);

        emit Transfer(sender, recipient, leftTokens);
...
contract CoreY is ERC20, ERC20Detailed {
  ...
  address public owner;
    ...
    /* Price of a CoreY token, in 'wei' denomination */

    uint constant private TOKEN_PRICE_IN_WEI = 20;
  ...
  constructor () public ERC20Detailed("CoreY.Finance", "CoreY", 18) {

      owner = msg.sender;

    _totalSupply = 5500 *(10**uint256(18));
