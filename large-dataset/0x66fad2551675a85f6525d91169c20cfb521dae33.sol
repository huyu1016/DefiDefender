...
contract ProtocolCompound is IIProtocol, IIOracle, CompoundUtils {
    ...
    function pnl(IERC20 collateral, IERC20 debt, uint256 leverage) public returns(uint256) {

        return _getPrice(collateral).mul(collateralAmount(collateral)).mul(leverage - 1)

            .mul(1e18)

            .div(

                _getPrice(debt).mul(borrowAmount(debt)).mul(leverage)
