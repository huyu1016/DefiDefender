...
**          yomama.finance (YOMA) presale contract

**          Yomama so fat, she was deployed spread out over 4 contracts 
...
contract YOMAPresale {

    using SafeMath for uint256;

    

    address payable private _presaleVaultAddress;

    address private _contractAddress;

    uint256 public remainingEthInWei;
    ...
    bool public hasPresaleOpened;

    

    constructor(address yoma_contract_address, address payable presale_address) public {

        owner = msg.sender;

        _contractAddress = yoma_contract_address;

        _presaleVaultAddress = presale_address;



        remainingEthInWei = 350 * 1e18;

        TOKENS_PER_ETH = 20;

        remainingTokens = remainingEthInWei.mul(TOKENS_PER_ETH);



        hasPresaleOpened = false;
    ...
    function openPresale() external {
        ...
        require(msg.sender == owner, "YOMA owner can open presale");

        hasPresaleOpened = true;

    }

    

    function _buyTokens(address payable _toAddress) internal{

        require(hasPresaleOpened, "Presale not yet open!");

        

        // set wei amount to fill for the last purchase

        uint256 weiAmount = remainingEthInWei < msg.value ? remainingEthInWei : msg.value;

        

        // send ether to the vault

        _presaleVaultAddress.transfer(weiAmount);

        

        // buy tokens

        uint256 tokenAmount = weiAmount.mul(TOKENS_PER_ETH);

        releaseTokens(_toAddress, tokenAmount);

        

        // update remaining

        remainingEthInWei = remainingEthInWei.sub(weiAmount);
    ...
    function releaseTokens(address receiver, uint256 amount) internal {
        ...
        Yomama yoma = Yomama(_contractAddress);

        yoma.transferFrom(_presaleVaultAddress, receiver, amount);
