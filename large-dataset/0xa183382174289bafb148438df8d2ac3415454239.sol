...
contract SavagePenguinSociety is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress;

    uint256 public SPS_MAX = 10000;
    ...
    uint256 private PUBLIC_MINT_LIMIT = SPS_MAX;
    ...
    Counters.Counter private _publicSPS;

    constructor() ERC721("Savage Penguin Society", "SPS") {}
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicSPS.current();

                if (_publicSPS.current() < SPS_MAX) {
                    _publicSPS.increment();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicSPS.current() < SPS_MAX,"Purchase would exceed SPS_MAX");
