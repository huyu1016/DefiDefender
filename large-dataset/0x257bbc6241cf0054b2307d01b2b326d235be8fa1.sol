...
contract SaffronStrategy is ISaffronStrategy {
  ...
  constructor(address _sfi_address, address _team_address) {
  ...
  function wind_down_epoch(uint256 epoch) external {
    ...
    require(epoch == 1, "v1: only epoch 1");
    ...
    uint256 dSFI = (22500 * 1 ether) >> epoch; // 11250 SFI in epoch 1
    ...
    uint256 lpSFI = (7500 * 1 ether) >> epoch; // 3750 SFI in epoch 1
  ...
  function wind_down_pool(uint256 pool, uint256 epoch) external {
    ...
    require(msg.sender == governance, "must be gov");
  ...
  function deploy_all_capital() external override {
    ...
    for (uint256 i = 0; i < pools.length; i++) {
      ...
      if (base_asset.balanceOf(pools[i]) > 0) pool.hourly_strategy(adapters[0]);
  ...
  function delete_adapters() external override {
    ...
    require(block.timestamp > epoch_cycle.start_date + 5 weeks, "v1: must be completed");
