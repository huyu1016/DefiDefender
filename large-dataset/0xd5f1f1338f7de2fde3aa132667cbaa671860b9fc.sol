...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    address public buyerRewardPool;
    ...
    constructor() public {
        ...
        nextRebalance = lastRebalance + rebalanceInterval;

        lpUnlocked = block.timestamp + 365 days;

        minRebalanceAmount = 100 ether;

        treasury = msg.sender;

        buyerRewardPool = 0x2C9824f26194279c39a25Ef7F90b2114EA91FD28;
    ...
    function _transfer(
        ...
        if (
            ...
            feelessAddr[from] == false &&

            feelessAddr[to] == false

        ) {

            if (from == uniswapV2Pair) {

                // when buying token, apply the buy reward

                uint256 buyerReward = balanceOf(buyerRewardPool).mul(amount).div(totalSupply());

                super._transfer(from, to, amount);

                super._transfer(buyerRewardPool, to, buyerReward);

            } else {

                // when transferring 2% to Liquidty Providers, 2% to buyerRewardPool

                uint256 liquidityLockAmount = amount.div(liquidityLockDivisor);

                uint256 buyerRewardFeeAmount = amount.div(liquidityLockDivisor);

                

                super._transfer(from, address(this), liquidityLockAmount);

                super._transfer(from, buyerRewardPool, buyerRewardFeeAmount);

                super._transfer(from, to, amount.sub(liquidityLockAmount).sub(buyerRewardFeeAmount));
    ...
    function getPathForETHtoACHY() private view returns (address[] memory) {
    ...
    function rebalanceLiquidity() public {
        ...
        require(tokenPriceEnabled,"The token is not listed in Uniswap.");

        require(

            balanceOf(msg.sender) >= minRebalanceAmount,

            "You are not part of Anarchy."
        ...
        uint price=IUniswapV2Router02(uniswapV2Router).getAmountsOut(1,getPathForETHtoACHY())[1];

        require(price < lastPrice,"Rebalancing only when the token price is down.");
        ...
        uint256 amountToRemove = ERC20(uniswapV2Pair)

            .balanceOf(address(this))

            .div(rebalanceDivisor);
        ...
        uint256 _locked = balancer.rebalance(callerRewardDivisor);
    ...
    function remLiquidity(uint256 lpAmount)
...
contract Balancer {
    ...
    function rebalance(uint256 callerRewardDivisor) external returns (uint256) {
    ...
    function swapEthForTokens(uint256 EthAmount, uint256 callerRewardDivisor)
...

    {
        ...
        IUniswapV2Router02(token.uniswapV2Router())
            ...
            .value(EthAmount.sub(treasuryAmount))(
...
contract AnarchyToken is

    ERC20(50000 ether),

    ERC20Detailed("Anarchy", "ACHY", 18),
...

{
    ...
    function setTokenPriceFlag(bool _tokenPriceFlag)
...

    {
        ...
        tokenPriceEnabled = _tokenPriceFlag;

    }

    

    function setBuyerRewardPool(address _buyerRewardPool)
...

    {
        ...
        buyerRewardPool = _buyerRewardPool;
...

    {
        ...
        require(
            ...
            "ACHY::setUniswapV2Router: already set"
    ...
    function setUniswapV2Pair(address _uniswapV2Pair)
...

    {
        ...
        require(
            ...
            "ACHY::setUniswapV2Pair: already set"
    ...
    function setCallerRewardDivisior(uint256 _rewardDivisor)
...

    {
        ...
        if (_rewardDivisor != 0) {
            ...
            require(
                ...
                "ACHY::setCallerRewardDivisor: too small"
