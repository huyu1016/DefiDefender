...
contract MetaStonezGrowableNFT is Ownable, ERC721Checkpointable, IDelegatedMintable {
  using Strings for uint256;

  /**
   * @notice Sender is not allowed to mint.
   */
  error NotAuthorized();
  /**
   * @notice You tried to mint a non-existent token.
   */
  error NonexistentToken();
  /**
   * @notice You tried to create a token using an existing token ID.
   */
  ...
  /**
   * @notice A mapping of token IDs to metadata URIs
   */
   ...
   *
   * New tokens can only be minted by authorized contracts, and
   * only if the token has a URI set and has not already been minted.
   *
  ...
  function mint(address recipient, uint256 tokenId) external override {
    ...
    // no check to see if tokenId was already minted
    // since _safeMint does that
    totalSupply += 1;
  ...
  function mintMany(address recipient, uint256 tokenIdStart, uint256 count) external override {
    ...
    for (uint256 i = 0; i < count; i++) {
      if (bytes(tokenURIs[tokenIdStart + i]).length == 0) {
        revert NonexistentToken();
      }
  ...
  /**
   * @notice Adds a new token with a specific URI.
   *
   * If the token ID is already in use then this call will revert.
   * This means that the URI can only be set once, so be sure it is correct.
   */
  function addToken(
    uint256 tokenId,
    string calldata uri
  ...
  /**
   * @notice Set the URI for a specific token. Can **NOT** be called after the token is minted.
   */
  function editToken(uint256 tokenId, string calldata uri) external onlyOwner {
    if (_exists(tokenId)) {
      revert TokenAlreadyExists();
    }
