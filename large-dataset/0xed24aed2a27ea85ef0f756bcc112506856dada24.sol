...
// File: contracts/NFTS_Genesis.sol
...
contract NFT_Genesis is ERC721, ERC2981ContractWideRoyalties, Ownable
{
    using Strings for uint256;
    ...
    uint256 public PRE_PRICE = 0.12 ether;
    ...
    constructor(string memory baseURI, uint256 _presaleStartDate, uint256 _presaleEndDate, uint256 _publicsaleStartDate, string memory _unrevealedURI, uint256 _revealDate) ERC721("NFT Stadium Genesis Collection", "NSGC") {
        setBaseURI(baseURI);
    ...
    function mintNFTs(uint256 _count) public payable {
        ...
        require(block.timestamp > presaleStartDate, "Pre/public sale not started yet!");
        if (block.timestamp > presaleEndDate) {
            require(block.timestamp > publicsaleStartDate, "Public sale not started yet!");
        }
        ...
        if (block.timestamp < presaleEndDate) {
            ...
            require(preMinted[msg.sender] <= 4, "Address can't mint more NFTs in pre-sale.");
        } else {
            minted[msg.sender] += _count;
            require(minted[msg.sender] <= 4, "Address can't mint more NFTs in public-sale.");
        }
    ...
    function setPresaleStartDate(uint256 _startDate) external onlyOwner {
