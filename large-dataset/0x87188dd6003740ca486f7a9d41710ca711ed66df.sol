...
contract ArabianCamels is ERC721Enumerable, Ownable, IERC2981 {
    using Strings for uint256;
    ...
    uint256 public maxWhiteListSale = 4000;
    uint256 public maxPublicSale = 6000;
    uint256 public preSaleStart = 1642197600;
    uint256 public whiteListSaleStart = 1643839200;
    uint256 public publicSaleStart = 1646258400;
    ...
    function mint(uint256 mintAmount) external payable {
        ...
        require(mintAmount <= maxMintAmount, "Mint amount should be less than max mint amount");
        require(balanceOf(msg.sender) + mintAmount <= maxMintAmount, "Balance of minter has greater than 25 Antara Movie NFTs");
        uint256 supply = totalSupply();
        require(supply + mintAmount <= maxSupply, "Max supply overflow");
        
        if (block.timestamp < whiteListSaleStart) {
            require(totalSupply() + mintAmount <= maxPreSale, "Only 2000 tokens are allowed for presale");
            require(msg.value == cost * mintAmount, "wrong ETH value");
        } else if (block.timestamp < publicSaleStart) {
            require(!onlyWhiteList || isWhitelisted(msg.sender), "Account is not whitelisted");
            require(totalSupply() + mintAmount <= maxWhiteListSale, "Max token amount is 4000 for whitelisted users");
            require(msg.value == cost * mintAmount, "wrong ETH value");
        } else {
            require(totalSupply() + mintAmount <= maxPublicSale, "All public sale NFTs have been sold out!");
            require(msg.value == cost * mintAmount, "wrong ETH value");
        }
    ...
    function setMaxWhiteListSale(uint256 max) external onlyMultisig {
        maxWhiteListSale = max;
    }

    function setMaxPublicSale(uint256 max) external onlyMultisig {
        maxPublicSale = max;
    }
    ...
    function isPreSaleWhitelisted(address user) public view returns (bool) {
    ...
    function setCost(uint256 newCost) external onlyMultisig {
        cost = newCost;
    }
    ...
    function setMaxMintAmount(uint256 newmaxMintAmount) external onlyMultisig {
    ...
    function setDates(uint256 _preSaleStart, uint256 _whiteListSaleStart, uint256 _publicSaleStart) external onlyMultisig {
    ...
    function preSaleWhitelistUsers(address[] calldata users) external onlyOwner {
        for (uint256 i; i < users.length; i++) {
    ...
    function deleteFromPreSaleWhitelist(address user) external onlyOwner {
        preSaleWhitelist[user] = false;
    }
