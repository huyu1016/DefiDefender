...
contract PrivilegedTigerClub is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x1c4f7BD9F9456b49aE1B27B80caBdF43184b7569);

    uint256 public PTC_MAX = 8888;
    uint256 private PRESALE_PRICE = 0.1 ether;
    ...
    Counters.Counter private _publicPTC;

    constructor() ERC721("Privileged Tiger Club", "PTC") {}
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicPTC.current();

                if (_publicPTC.current() < PTC_MAX) {
                    _publicPTC.increment();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicPTC.current() < PTC_MAX,"Purchase would exceed PTC_MAX");
