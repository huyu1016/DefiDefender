...
// File: contracts/component/Whitelist.sol
...
contract Whitelist is HasOwner
{
     ...
     * @dev The constructor.
     *
     * @param _owner The address of the owner, which can add/remove entries ot the whitelist.
     */
    constructor(address _owner) public
        HasOwner(_owner)
    {
    ...
    /**
     * @dev Public function that allows the owner to add multiple entries to whitelist
     *
    ...
    /**
     * @dev Public function that allows the owner to remove multiple entries from the whitelist (i.e. blacklist them)
     *
...
// File: contracts/trait/HasWhitelist.sol

/**
 * @title HasWhitelist
 *
 * @dev Adds whitelist component to smart contracts.
 */
contract HasWhitelist is HasOwner {
    ...
    /**
     * @dev Methods allows the owner to change whitelist component address
     *
     * @param _address The address of the new whitelist component
     */
...
// File: contracts/fundraiser/WhitelistFundraiser.sol

/**
 * @title WhitelistFundraiser
 *
 * @dev A fundraiser that has a whitelist and allows only whitelisted addresses to participate in the fundraiser
 */
contract WhitelistFundraiser is HasWhitelist, BasicFundraiser {
    /**
     * @dev Override default function to allow only whitelisted addresses to participate in the fundraiser.
     */
...
/**
 * @title EtalonGlobalSeriesAToken
 */

contract EtalonGlobalSeriesAToken is MintableToken, PausableToken {
    constructor(address _owner, address _minter)
        StandardToken(
            "Etalon Global Series A",   // Token name
            "ETLA", // Token symbol
            18  // Token decimals
        )
        HasOwner(_owner)
...
/**
 * @title EtalonGlobalSeriesATokenSafe
 */

contract EtalonGlobalSeriesATokenSafe is TokenSafe {
  constructor(address _token)
    TokenSafe(_token)
    public
  {
    // Group "Etalon Global Core Team"
    init(
      1, // Group Id
      1538787600 // Release date = 2018-10-06 01:00 UTC
    );
    add(
      1, // Group Id
      0xA1f82F85e85459db3f83510d8FE4750B69A13889,  // Token Safe Entry Address
      50000000000000000000000000  // Allocated tokens
    );
    add(
      1, // Group Id
      0x521452F17ceECC6BD5739CbE30B165C6010A14d2,  // Token Safe Entry Address
      50000000000000000000000000  // Allocated tokens
    );
  }
}
...
/**
 * @title EtalonGlobalSeriesATokenFundraiser
 */

contract EtalonGlobalSeriesATokenFundraiser is MintableTokenFundraiser, PresaleFundraiser, IndividualCapsFundraiser, CappedFundraiser, RefundableFundraiser, WhitelistFundraiser, GasPriceLimitFundraiser {
    EtalonGlobalSeriesATokenSafe public tokenSafe;

    constructor()
        HasOwner(msg.sender)
        public
    {
        token = new EtalonGlobalSeriesAToken(
        msg.sender,  // Owner
        address(this)  // The fundraiser is the minter
        );

        tokenSafe = new EtalonGlobalSeriesATokenSafe(token);
        MintableToken(token).mint(address(tokenSafe), 100000000000000000000000000);

        initializeBasicFundraiser(
            1543626000, // Start date = 2018-12-01 01:00 UTC
            1551398400,  // End date = 2019-03-01 00:00 UTC
            700, // Conversion rate = 700 ETLA per 1 ether
            0xdFD2f6a3d6b60A5332a14C3d783c2cE80d855A22     // Beneficiary
        );

        initializeIndividualCapsFundraiser(
            (0.2 ether), // Minimum contribution
            (1000 ether)  // Maximum individual cap
        );
        ...
        initializePresaleFundraiser(
            ...
            1538740800, // Start = 2018-10-05 12:00 UTC
            1543622400,   // End = 2018-12-01 00:00 UTC
            1000
        );

        initializeCappedFundraiser(
            (100000 ether) // Hard cap
        );
        ...
        initializeWhitelist(
            msg.sender  // Fundraiser creator is the whitelist owner
        );
    }
