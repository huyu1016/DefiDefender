...
// File: contracts/ISaiIDaiSwap.sol
...
contract ISaiIDaiSwap is Pausable, Destructible, IErc20Swap
...
//    require(cdaiSwap.cToken().underlying() == address(saidaiSwap.dai()), "DAI tokens mismatch");
  ...
  function getRate(address src, address dst, uint256 /*srcAmount*/) external view

  returns(uint expectedRate, uint slippageRate)  // real rate = returned value / 1e18

  {

    if (src == address(isaiSwap.loanToken()) && dst == address(idaiSwap.loanToken())) {
 ...
 */
...

  {
    ...
    require(
      ...
      _userOfferTokenAddress == address(isaiSwap.loanToken()),
...

  {
    ...
    isai.transferFrom(msg.sender, address(this), srcAmount);



    isai.approve(address(isaiSwap), srcAmount);

    isaiSwap.swapToken(address(isai), srcAmount);
    ...
    require(cdaiBalance > 0, "iDAI balance is 0");



    require(idai.transfer(msg.sender, cdaiBalance), "Cannot tranfer iDAI to the msg.sender address");
