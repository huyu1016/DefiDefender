...
contract Ownable {
    ...
    modifier onlyOwner() {
        ...
        require(msg.sender == owner, "Owner is incorrect");
...
contract BirthdayCake is Ownable {
    ...
    string public name = "BirthdayCakePieces";
   ...
   uint8 constant internal partyFee_ = 100; //party fund, This value will be divided by 10 

    uint8 constant internal DailyInterest_ = 1; 

    uint8 constant internal IntFee_ = 10; //Vault Fund This value will be divided by 10
    ...
    address party = 0xe1cDA441ffA203eCA692E3398f3C3346Ee2B786e;
        ...
        uint256 partyFee1 = msg.value.div(100).mul(partyFee_);

        uint256 partyFeeFinal = SafeMath.div(partyFee1, 10);

        party.transfer(partyFeeFinal);
    ...
    function sell(uint256 _amountOfTokens) onlyBagholders public {
        ...
        uint256 _partyexit = SafeMath.div(SafeMath.mul(_ethereum, 5), 100);

        uint256 _taxedEthereum1 = SafeMath.sub(_ethereum, _dividends);

        uint256 _taxedEthereum = SafeMath.sub(_taxedEthereum1, _partyexit);

        uint256 _partyexitindividual = SafeMath.div(SafeMath.mul(_ethereum, partyFee_), 100);

        uint256 _partyexitindividual_final = SafeMath.div(_partyexitindividual, 10);
        ...
        party.transfer(_partyexitindividual_final); 
    ...
    function sellPrice() public view returns (uint256) {
        ...
        // the calculation relies on the token supply, so we need supply. Doh.
    ...
    function buyPrice() public view returns (uint256) {
        ...
        if (tokenSupply_ == 0) {
            ...
            uint256 _taxedEthereum = SafeMath.add(_taxedEthereum1, _partyexit);
    ...
    function calculateTokensReceived(uint256 _ethereumToSpend) public view returns (uint256) {
        ...
        uint256 _partybuyfees = SafeMath.div(SafeMath.mul(_ethereumToSpend, 5), 100);

        uint256 _taxedEthereum1 = SafeMath.sub(_ethereumToSpend, _dividends);

        uint256 _taxedEthereum = SafeMath.sub(_taxedEthereum1, _partybuyfees);
  ...
  function purchaseTokens(uint256 _incomingEthereum, address _referredBy) internal returns (uint256) {
        ...
        uint256 _partybuyfees = SafeMath.div(SafeMath.mul(_incomingEthereum, 5), 100);

        uint256 _dividends1 = SafeMath.sub(_undividedDividends, _referralBonus);

        uint256 _dividends = SafeMath.sub(_dividends1, _partybuyfees);
