...
contract BRO is Ownable {
    ...
    // multiplier that will be applied all amounts inside contract

    uint public constant MULTIPLIER = 10**6;
    ...
    function depositLoan(address _token, uint _amount) public isAcceptableTokens(_token) returns (bool) {
        ...
        uint quarterCapitalReimbursed = _amount.mul(MULTIPLIER).div(2 * quarterCount);

        uint quarterInterests = _amount.mul(MULTIPLIER).mul(interestRate).div(100).div(quarterCount);



        RewardPlan storage rewardPlan = rewardPlans[msg.sender];

        rewardPlan.repaymentUsdAmount = _amount.mul(MULTIPLIER).div(2);
     ...
     * @param _token    address  Address of stablecoin to be sent

     * @param _receiver address  Address of recipient

     * @param _amount   uint     Amount to be sent, this amount is 10**6 multiplied to original amount
    ...
    function sendStableCoin(address _token, address payable _receiver, uint _amount) internal isAcceptableTokens(_token) returns (uint) {
        ...
        if (_token == USDT_ADDRESS) {
            ...
            // USDt has 6 decimals so wei amount is same
            ...
            require(token.transfer(_receiver, _amount.mul(10**12)), "Failed transfering stablecoin");
    ...
    function mintToz(address _receiver, uint _amount) internal returns (uint) {
        ...
        uint weiAmount = _amount.mul(10**12);
