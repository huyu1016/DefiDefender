...
interface IMarketplace {
    ...
    event OrderUpdated(bytes32 id, uint256 priceInWei);
...
// File contracts/EmillionMarketplace.sol
    ...
    ERC721Holder
{
    ...
    Iemillionnft public iemillionnft;
    address public WETH;
    ...
    constructor(address _nft, address _WETH) {
        WETH = _WETH;
        iemillionnft = Iemillionnft(_nft);
    }
    ...
    function createOrder(
        ...
        _createOrder(_nftAddress, _assetId, _priceInWei);
    }
    ...
    function getArtistAddress(uint256 _id) public view returns (address) {
        return iemillionnft.getArtist(_id);
    }
    ...
    function purchaseOrder(address _nftAddress, uint256 _assetId)
        public
        payable
        whenNotPaused
    {
        IWETH(WETH).deposit{value: msg.value}();
        _purchaseOrder(_nftAddress, _assetId, msg.value);
    }
    ...
    function _purchaseOrder(
        ...
        require(
            ...
            "Marketplace: bid should be = order price"
        );

        // Create bid
        bytes32 bidId = keccak256(
            abi.encodePacked(block.timestamp, msg.sender, order.id, _priceInWei)
        );

        IWETH(WETH).transfer(order.seller, (order.price).mul(9500).div(10000));
        IWETH(WETH).transfer(
            getArtistAddress(_assetId),
            (order.price).mul(375).div(10000)
        );
        // IWETH(WETH).transfer(address(this), (order.price).mul(125).div(10000));

        _executeOrder(order.id, msg.sender, _nftAddress, _assetId, order.price);

        emit purchaseOrderEvent(
            bidId,
            _nftAddress,
            _assetId,
            msg.sender,
