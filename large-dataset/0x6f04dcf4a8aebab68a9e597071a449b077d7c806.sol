...
contract EternalCollections is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0xe9E7aEb2dC214C8Fe354feb1ECd61871Ea201539);

    uint256 public MAX_SUPPLY = 3333;
    uint256 private PRESALE_PRICE = 0.08 ether;
    uint256 private PUBLIC_PRICE = 0.128 ether;
    
    uint256 private PRESALE1_MINT_LIMIT = 35;
    uint256 private PRESALE2_MINT_LIMIT = 5;
    uint256 private PUBLIC_MINT_LIMIT = MAX_SUPPLY;
    uint256 private PRESALE1_PURCHASE_LIMIT = 35;
    uint256 private PRESALE2_PURCHASE_LIMIT = 5;
    uint256 private PUBLIC_PURCHASE_LIMIT = 10;
    uint256 private POOL_PERCENT = 5;
    
    bytes32 private _whitelist1Root;
    bytes32 private _whitelist2Root;

    mapping(address => uint256) private _mappingPresale1MintCount;
    mapping(address => uint256) private _mappingPresale2MintCount;
    mapping(address => uint256) private _mappingPublicMintCount;

    uint256 private _presale1DateTime = 1645459200; // (GMT): Monday, February 21, 2022 4:00:00 PM
    
    uint256 private _presale2DateTime = 1647786120; // (GMT): Sunday, March 20, 2022 2:22:00 PM
    
    uint256 private _publicDateTime = 1647958920; // (GMT): Tuesday, March 22, 2022 2:22:00 PM
    uint256 private _revealDateTime = 1711117320;
    ...
    constructor() ERC721("Eternal Collections", "EC-Spring") {}
    ...
    function setPresale1DateTime(uint256 presale1DateTime) external onlyOwner {
        _presale1DateTime = presale1DateTime;
    }

    function setPresale2DateTime(uint256 presale2DateTime) external onlyOwner {
        _presale2DateTime = presale2DateTime;
    }
    ...
    function setWhiteList1Root(bytes32 root1) external onlyOwner {
        _whitelist1Root = root1;
    }

    function setWhiteList2Root(bytes32 root2) external onlyOwner {
        _whitelist2Root = root2;
    }
    ...
    function isWhiteListed1(bytes32 _leafNode, bytes32[] memory proof) public view returns (bool) {
        return MerkleProof.verify(proof, _whitelist1Root, _leafNode);
    }

    function isWhiteListed2(bytes32 _leafNode, bytes32[] memory proof) public view returns (bool) {
        return MerkleProof.verify(proof, _whitelist2Root, _leafNode);
    }
    ...
    function setMintLimit(uint256 presale1MintLimit, uint256 presale2MintLimit, uint256 publicMintLimit) external onlyOwner {
        PRESALE1_MINT_LIMIT = presale1MintLimit;
        PRESALE2_MINT_LIMIT = presale2MintLimit;
        PUBLIC_MINT_LIMIT = publicMintLimit;
    }

    function setPurchaseLimit(uint256 presale1PurchaseLimit, uint256 presale2PurchaseLimit, uint256 publicPurchaseLimit) external onlyOwner {
        PRESALE1_PURCHASE_LIMIT = presale1PurchaseLimit;
        PRESALE2_PURCHASE_LIMIT = presale2PurchaseLimit;
    ...
    function presaleRound1(uint256 numberOfTokens, uint256 index, uint256 amount, bytes32[] calldata proof) external payable {
        require(_publicCounter.current() + numberOfTokens <= MAX_SUPPLY,"Purchase would exceed MAX_SUPPLY");

        if (msg.sender != owner()) {
            
            require(isWhiteListed1(toLeaf(msg.sender, index, amount), proof), "Invalid proof");

            require(_presale1DateTime <= block.timestamp,"Presale Round1 is not activated.");

            require(numberOfTokens <= PRESALE1_PURCHASE_LIMIT,"Can only mint up to purchase limit");

            require(PRESALE_PRICE * numberOfTokens <= msg.value,"ETH amount is not sufficient");

            _mappingPresale1MintCount[msg.sender] = _mappingPresale1MintCount[msg.sender] + numberOfTokens;
            
            require(_mappingPresale1MintCount[msg.sender] <= PRESALE1_MINT_LIMIT,"Overflow for PRESALE1_MINT_LIMIT");
    ...
    function presaleRound2(uint256 numberOfTokens, uint256 index, uint256 amount, bytes32[] calldata proof) external payable {
        require(_publicCounter.current() + numberOfTokens <= MAX_SUPPLY,"Purchase would exceed MAX_SUPPLY");

        if (msg.sender != owner()) {
                
            require(isWhiteListed2(toLeaf(msg.sender, index, amount), proof), "Invalid proof");

            require(_presale2DateTime <= block.timestamp,"Presale Round2 is not activated.");

            require(numberOfTokens <= PRESALE2_PURCHASE_LIMIT,"Can only mint up to purchase limit");

            require(PRESALE_PRICE * numberOfTokens <= msg.value,"ETH amount is not sufficient");

            _mappingPresale2MintCount[msg.sender] = _mappingPresale2MintCount[msg.sender] + numberOfTokens;
            
            require(_mappingPresale2MintCount[msg.sender] <= PRESALE2_MINT_LIMIT,"Overflow for PRESALE2_MINT_LIMIT");
    ...
    function processRound() external onlyOwner {
