...
contract LidCertifiedPresale is Initializable, Ownable, ReentrancyGuard {
    ...
    function deposit(address payable referrer) public payable whenPresaleActive nonReentrant {
        ...
        uint tokensToIssue = depositVal.div(calculateRatePerEth()).mul(10**18);
    ...
    function calculateRatePerEth() public view returns (uint) {
        ...
        return totalTokens.mul(multiplierPrice).add(startingPrice);
    ...
    function _repairAccount(address account, uint lidAtPurchase) internal onlyOwner returns (uint, bool) {
        ...
        uint correctEthPrice = lidAtPurchase.mul(multiplierPrice).add(startingPrice);

        uint correctLid = depositAccounts[account].div(correctEthPrice).mul(10**18);
