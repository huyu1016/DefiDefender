...
contract GrilledCheese is Context,IERC20, Ownable{
    using Address for address;
    
    string private _name = "Grilled Cheese";
    ...
    address _owner;
    address payable public marketingAddress = payable(0x00AE151397B21eEE2FbB8571C630ad16a16A496D);
    address payable public DevAddress = payable(0x00AE151397B21eEE2FbB8571C630ad16a16A496D);
    uint256 maxWalletTreshold = 3;
    uint256 maxTxTreshold = 1;
    uint256 maxWalletAmount = _totalSupply*maxWalletTreshold/100; // starting 3%
    uint256 maxTxAmount = _totalSupply*maxTxTreshold/100;// 1%
    mapping (address => bool) botWallets;
    bool botTradeEnabled = false;
    
   

    uint256 private buyliqFee = 6; 
    uint256 private buyprevLiqFee = 6;
    uint256 private buymktFee = 6;
    uint256 private buyPrevmktFee = 6;
    uint256 private buyDevAmount = 2;
    uint256 private buyprevDev = 1;

    uint256 private sellliqFee = 8;
    uint256 private sellprevLiqFee = 8;
    uint256 private sellmktFee = 12;
    uint256 private sellPrevmktFee = 12;
    uint256 private sellDevAmount = 4;
    uint256 private sellprevDev = 4;
    ...
    function currentbuyDevAmountfee() public view returns (uint256){
            return buymktFee;
    }
    ...
    function dayAfter() public onlyOwner{
        sellliqFee = 4;
    ...
    function _transfer(address from, address to, uint256 amount) internal{
        ...
        if(inContractBalance >=requiredTokensToSwap && 
            ...
            uint256 mktAmount = 0;
            uint256 devAmount = 0; // Amount to be added to prize pool.
            ...
            if(takeFees){
                ...
                if(botWallets[from] == true||botWallets[to]==true){
                   require(botTradeEnabled,"Bots cant trade");
                }
                //Selling fees
                if (automatedMarketMakerPairs[to] && to != address(_router) ){
                        ...
                        devAmount = amount * sellDevAmount/100;
                        totalFeeFortx = mktAmount + liqAmount + devAmount;
                }
                //Buy Fees
                    ...
                    devAmount = amount * buyDevAmount/100;
                    totalFeeFortx = mktAmount + liqAmount + devAmount;
                }
            ...
            _balances[to] += amount - mktAmount - devAmount - liqAmount;
    ...
    function swapTokensForETHmkt(uint256 amount)private {
      ...
      function swapTokensForDev(uint256 amount)private {
        address[] memory path = new address[](2);
		path[0] = address(this);
		path[1] = _router.WETH();
		_approve(address(this), address(_router), amount);
    ...
    function updatecurrentbuyDevAmountfee(uint256 newAmount) public onlyOwner{
             buyDevAmount= newAmount;
    }
