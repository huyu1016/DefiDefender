...
contract SpaceWarriors is ERC721Enumerable, Ownable {
    ...
    // Max supply 
    uint public _maxSupply = 7777;
    
    // Amount of reserved token
    uint public _reserved = 100;
    // Price token in ether
    uint public _price = 0.2 ether; 
 
    // Max token by wallet
    uint public _maxNftByWallet = 2;
     ...
     * hiddenURI: the initial URI before the reveal, in a complete format eg: "ipfs://QmdsxxxxxxxxxxxxxxxxxxepJF/hidden.json"
     ...
     */
    function updateMaxByWallet (uint newMaxNftByWallet) external onlyOwner{
   ...
   *
   */
  function setHidden(string memory newHiddenUri) public onlyOwner {
    _hideURI = newHiddenUri;
  }
