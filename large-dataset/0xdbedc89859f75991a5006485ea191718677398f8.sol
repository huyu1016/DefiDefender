...
contract CapitalPools is Ownable, Initializable {
    ...
    uint256 public MIN_BLOCK_REWARD = 10**18;
    ...
    uint256 public salesmanRewardRatio; // Percentage of salesman earning the seller\u2019s mining share
    ...
    mapping (address => address) public salesmanRelation;
    ...
    function initialize(HPPToken _token, uint256 _initPerBlock, uint256 _startBlock, uint256 _reduceBlockCount, address _master, address _devAddr) public onlyOwner initializer {

        token = _token;

        initPerBlock = _initPerBlock;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyMaster {
        ...
        uint256 lastReduceBlock = startBlock;

        uint256 perBlock = initPerBlock;

        if (poolInfo.length > 0) {

            PoolInfo storage pool = poolInfo[0];

            lastReduceBlock = pool.lastReduceBlock;
        ...
        poolInfo.push(PoolInfo({
            ...
            lastReduceBlock:lastReduceBlock,
    ...
    // Can only be called by the owner in the before starting, or the controller can call after starting.
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyMaster {
        ...
        require(totalAllocPoint > 0, "totalAllocPoint can not zero.");
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            (reward, afterPerBlock, afterReduceBlock) = calculateReward(pool);
        ...
        if (salesmanRewardRatio > 0) {
            ...
            pendingReward = pendingReward.sub(pendingReward.mul(salesmanRewardRatio).div(100));
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (afterReduceBlock != pool.lastReduceBlock) {

            pool.lastReduceBlock = afterReduceBlock;

        }

        if (afterPerBlock != pool.perBlock) {

            pool.perBlock = afterPerBlock;
        ...
        if (block.number - startBlock <= 518400) {

            uint256 devReward = reward.div(10);

            token.mint(devAddr, devReward);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _salesman) public {
        ...
        if (user.amount > 0) {
            ...
            pending = sendSalesmanReward(msg.sender, pending);
        ...
        if (_salesman != address(0x0) && _salesman != msg.sender && salesmanRelation[msg.sender] == address(0x0)) {

            salesmanRelation[msg.sender] = _salesman;
    ...
    function sendSalesmanReward(address _user, uint256 _totalReward) internal returns (uint256) {

        if (salesmanRewardRatio == 0 || _totalReward == 0) {

            return _totalReward;

        }

        uint256 salesmanReward = _totalReward.mul(salesmanRewardRatio).div(100);

        address salesman = salesmanRelation[_user];

        if (salesman != address(0x0)) {

            address superior = salesmanRelation[salesman];

            if (superior == address(0x0)) {

                safeTokenTransfer(salesman, salesmanReward);

            } else {

                uint256 superiorReward = salesmanReward.mul(30).div(100);

                safeTokenTransfer(superior, superiorReward);

                safeTokenTransfer(salesman, salesmanReward.sub(superiorReward));
        ...
        return _totalReward.sub(salesmanReward);
    ...
    function calculateReward(PoolInfo storage pool) internal view returns (uint256, uint256, uint256) {

        uint256 segmentCount = (block.number.sub(pool.lastReduceBlock)).div(reduceBlockCount);

        uint256 reward = (block.number.sub(pool.lastRewardBlock)).mul(pool.perBlock).mul(pool.allocPoint).div(totalAllocPoint);

        uint256 afterPerBlock = pool.perBlock;

        uint256 afterReduceBlock = pool.lastReduceBlock;

        if (segmentCount > 0) {

            reward = (afterReduceBlock.add(reduceBlockCount).sub(pool.lastRewardBlock)).mul(afterPerBlock);
            ...
            for (uint256 i = 1 ; i <= segmentCount ; i++) {
                ...
                uint256 segmentEndCount = afterReduceBlock.add(reduceBlockCount.mul(i + 1));

                if (block.number < segmentEndCount) {

                    segmentBlockCount = block.number.sub(afterReduceBlock.add(reduceBlockCount.mul(i)));
    ...
    function perBlock() public view returns (uint256) {

        uint256 perBlockReward = initPerBlock;

        if (poolInfo.length > 0) {

            PoolInfo storage pool = poolInfo[0];

            perBlockReward = pool.perBlock;
