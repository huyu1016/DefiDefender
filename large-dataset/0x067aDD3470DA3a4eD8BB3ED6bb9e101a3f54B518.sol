...
contract Vault721 is Ownable, ReentrancyGuard {
    ...
    // string will be nftContract address->tokenId->owner address->bidder_address
    // mapping(address => mapping(uint256 => mapping(address => VaultItem)))
    ...
    struct Claim {
        ...
        //msg.value
        uint256 value;
        //if amount is 0, token type is 721
        uint256 amount;
        bytes data;
        bool fulfilled;
    }
    ...
    function approveSale(uint256 claimId) public onlyOwner{
        if(claims[claimId].amount>0){
            //if claim is for 1155
            // transfer NFT between buyer and bidder
