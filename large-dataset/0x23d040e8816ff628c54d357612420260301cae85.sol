...
contract THORChain_Router {
    ...
    event TransferOutAndCall(address indexed vault, address target, address asset, uint amount, address finalAsset, address to, uint256 amountOutMin, string memo);
    ...
    // Any vault calls to transferAndCall on a target contract that conforms with "swapOut(address,address)"
    // Example Memo: "TCA:ETH.0xFinalAsset:0xTo:"
    // Target comes from Bifrost aggregator whitelist for "TCA"
    // FinalAsset, To, amountOutMin come from originating memo
