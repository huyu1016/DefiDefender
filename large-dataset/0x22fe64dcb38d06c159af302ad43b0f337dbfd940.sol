...
contract KittyBountiesAdmin is Ownable, Pausable, ReentrancyGuard, COORole {
    ...
    /// @dev This event is fired whenever the owner changes the unsuccessfulBountyFeeInWCKWei.

    /// @param newUnsuccessfulBountyFeeInWCKWei  The UnsuccessfulBountyFee is paid by the original bounty creator if the bounty expires
    ...
    /// @dev The amount of fees collected (in wck_wei). This includes both fees to the contract owner and fees to any bounty referrers.
    ...
    ///  invalidate the bounty, but they are charged this flat fee to do so. This fee is measured in WCK_wei.
    ...
    /// @notice Sets the unsuccessfulBountyFeeInWCKWei value. If a bounty is still unfulfilled once the minimum number of blocks has passed,

    ///  an owner can withdraw the locked ETH. If they do so, this fee is deducted from the amount that they withdraw.

    /// @notice Only callable by the owner.

    /// @param _newUnsuccessfulBountyFeeInWCKWei  The unsuccessfulBountyFeeInWCKWei value to set (measured in WCK_wei).
...
/// @notice Once created, a bounty locks up WCK (wrapped-cryptokitties, the currency of CryptoKitties). Optionally,

/// the bounty creator may specify a number of blocks to "lock" their bounty, thus preventing them from being able

///  to cancel their bounty or withdraw their WCK until that number of blocks have passed. This guarantees a time
...
///  can create a bounty. This enables this contract to withdraw WCK from their account when the bounty is created,

///  and lock that WCK until the bounty is fulfilled or cancelled.
...
contract KittyBounties is KittyBountiesAdmin {
  ...
  struct Bounty {
    ...
    // The price (in WCK_wei) that a user will receive if they successfully fulfill this bounty.

    uint128 bountyPricePerCat;

    // The total value (in WCK_wei) that the bounty creator originally sent to the contract to create this
    ...
    /// @param bountyPricePerCat  The price (in WCK_wei) that a user will receive if they successfully fulfill
    ...
    /// @param unsuccessfulBountyFeeInWCKWei  The fee that is paid if the bounty is not fulfilled and the
    ...
    /// @param bountyPricePerCat  The price (in WCK_wei) that a user will receive if they successfully
    ...
    /// @param successfulBountyFeeInWCKWei  The fee that is paid when the bounty is fulfilled. This
    ...
    /// @param withdrawnAmount  The amount returned to the bounty creator (in WCK_wei). If the contract

    ///  is not frozen, then this is the total value originally sent to the contract minus

    ///  unsuccessfulBountyFeeInWCKWei. However, if the contract is frozen, no fees are taken, and the
    ...
    /// @param _totalAmountOfWCKToLock  A bounty creator specifies the total amount of WCK that they would like to

    ///  lock up for this bundle of bounties. If the quantity that they specified is greater than one,

    ///  then the totalValueIncludingFees per cat will be _totalAmountOfWCKToLock / _quantity
    ...
    ///  creator's original deposit, specified by unsuccessfulBountyFeeInWCKWei.
