...
contract KittyBountiesAdmin is Ownable, Pausable, ReentrancyGuard, COORole {
    ...
    /// @dev This event is fired whenever the owner changes the unsuccessfulBountyFeeInTBTCWei.

    /// @param newUnsuccessfulBountyFeeInTBTCWei  The UnsuccessfulBountyFee is paid by the original bounty creator if the bounty expires
    ...
    event UnsuccessfulBountyFeeInTBTCWeiUpdated(uint256 newUnsuccessfulBountyFeeInTBTCWei);
    ...
    /// @dev The amount of fees collected (in tbtc_wei). This includes both fees to the contract owner and fees to any bounty referrers.
    ...
    ///  invalidate the bounty, but they are charged this flat fee to do so. This fee is measured in TBTC_wei.

    uint256 public unsuccessfulBountyFeeInTBTCWei = 0;
    ...
    /// @dev The owner is not capable of changing the address of the WrappedKitties contract once the contract has been deployed.

    ///  This prevents an attack vector where the owner could change the payout ERC20 address to a worthless ERC20 contract, once

    ///  bounties were already locked in.

    address public tBTCAddress = 0x8dAEBADE922dF735c38C80C7eBD708Af50815fAa;
    ...
    /// @notice Sets the unsuccessfulBountyFeeInTBTCWei value. If a bounty is still unfulfilled once the minimum number of blocks has passed,

    ///  an owner can withdraw the locked ETH. If they do so, this fee is deducted from the amount that they withdraw.

    /// @notice Only callable by the owner.

    /// @param _newUnsuccessfulBountyFeeInTBTCWei  The unsuccessfulBountyFeeInTBTCWei value to set (measured in TBTC_wei).

    function setUnsuccessfulBountyFeeInTBTCWei(uint256 _newUnsuccessfulBountyFeeInTBTCWei) external onlyOwner {

        unsuccessfulBountyFeeInTBTCWei = _newUnsuccessfulBountyFeeInTBTCWei;

        emit UnsuccessfulBountyFeeInTBTCWeiUpdated(_newUnsuccessfulBountyFeeInTBTCWei);
    ...
    function withdrawFeeEarningsForAddress() external nonReentrant {
        ...
        require(ERC20(tBTCAddress).transfer(msg.sender, balance) == true, 'failed to transfer TBTC');
...
/// @notice Once created, a bounty locks up TBTC (wrapped-cryptokitties, the currency of CryptoKitties). Optionally,

/// the bounty creator may specify a number of blocks to "lock" their bounty, thus preventing them from being able

///  to cancel their bounty or withdraw their TBTC until that number of blocks have passed. This guarantees a time

///  period for bounty hunters to attempt to breed for a cat with the specified cattributes, generation, and/or

///  cooldown. This option is included since perhaps many breeders will not chase a bounty without this guarantee.

///  After that point, the bounty creator can withdraw their funds if they wish and invalidate the bounty, or they

///  can continue to leave the bounty active.

/// @notice The bounty creator must first call approve() in the WrappedCryptoKitties contract before they

///  can create a bounty. This enables this contract to withdraw TBTC from their account when the bounty is created,

///  and lock that TBTC until the bounty is fulfilled or cancelled.
...
contract KittyBounties is KittyBountiesAdmin {
  ...
  struct Bounty {
    ...
    uint256 geneMask;
        ...
        // A bounty creator specifies the earliest kittyID that they are willing to accept for this bounty.

        // When combined with a bounty creator specifying genes, this would enable a bounty creator to set

        // up a bounty for a particular fancy, even though fancy data is not stored on-chain. ID-range plus

        // genes is sufficient to determine that a kitty is a fancy, when the first and last ID's of a capped

        // fancy are known.

        uint128 earliestAcceptableIdInclusive;

        // A bounty creator specifies the latest kittyID that they are willing to accept for this bounty.
    ...
    // The price (in TBTC_wei) that a user will receive if they successfully fulfill this bounty.

    uint128 bountyPricePerCat;

    // The total value (in TBTC_wei) that the bounty creator originally sent to the contract to create this
    ...
    uint128 unsuccessfulBountyFeeInTBTCWei;
    ...
    /// @param bountyPricePerCat  The price (in TBTC_wei) that a user will receive if they successfully fulfill
    ...
    /// @param earliestAcceptableIdInclusive  A bounty creator specifies the earliest ID that they

    ///  would accept. They can submit 0 for this variable to be ignored.

    /// @param latestAcceptableIdInclusive  A bounty creator specifies the latest ID that they

    ///  would accept. They can submit uint128_max for this variable to be ignored.
    ...
    /// @param unsuccessfulBountyFeeInTBTCWei  The fee that is paid if the bounty is not fulfilled and the
    ...
    uint256 minBlockBountyValidUntil,
        ...
        uint256 unsuccessfulBountyFeeInTBTCWei
    ...
    /// @param bountyHunter  The address of the person who is fulfilling the bounty.

    /// @param bountyPricePerCat  The price (in TBTC_wei) that a user will receive if they successfully
    ...
    /// @param successfulBountyFeeInTBTCWei  The fee that is paid when the bounty is fulfilled. This
    ...
    uint256 bountyPricePerCat,
        ...
        uint256 successfulBountyFeeInTBTCWei
    ...
    /// @param withdrawnAmount  The amount returned to the bounty creator (in TBTC_wei). If the contract

    ///  is not frozen, then this is the total value originally sent to the contract minus

    ///  unsuccessfulBountyFeeInTBTCWei. However, if the contract is frozen, no fees are taken, and the
    ...
    /// @param _earliestAcceptableIdInclusive  A bounty creator specifies the earliest ID that they

    ///  would accept. They can submit 0 for this variable to be ignored.

    /// @param _latestAcceptableIdInclusive  A bounty creator specifies the latest ID that they
    ...
    /// @param _totalAmountOfTBTCToLock  A bounty creator specifies the total amount of TBTC that they would like to

    ///  lock up for this bundle of bounties. If the quantity that they specified is greater than one,

    ///  then the totalValueIncludingFees per cat will be _totalAmountOfTBTCToLock / _quantity

    function createBountyAndLockFunds(uint256 _geneMask, uint256 _genes, uint256 _earliestAcceptableIdInclusive, uint256 _latestAcceptableIdInclusive, uint256 _generation, uint256 _highestCooldownIndexAccepted, uint256 _minNumBlocksBountyIsValidFor, uint256 _quantity, uint256 _totalAmountOfTBTCToLock) external whenNotPaused {

        require(_totalAmountOfTBTCToLock >= unsuccessfulBountyFeeInTBTCWei.mul(uint256(2)), 'the value of your bounty must be at least twice as large as the unsuccessful bounty fee');
        ...
        require(ERC20(tBTCAddress).transferFrom(msg.sender, address(this), _totalAmountOfTBTCToLock) == true, 'failed to transfer TBTC from account of sender');



        uint256 totalValueIncludingFeesPerCat = _totalAmountOfTBTCToLock.div(_quantity);
        ...
        Bounty memory bounty = Bounty({
            ...
            totalValueIncludingFees: uint128(_totalAmountOfTBTCToLock),

            unsuccessfulBountyFeeInTBTCWei: uint128(unsuccessfulBountyFeeInTBTCWei),
        ...
        emit CreateBountyAndLockFunds(
            ...
            uint256(bounty.unsuccessfulBountyFeeInTBTCWei)
    ...
    function fulfillBountyAndClaimFunds(uint256 _bountyId, uint256 _kittyId, address _referrer) external whenNotPaused nonReentrant {
      ...
      uint256 genes;
        ...
        // By submitting 0 as the earliestAcceptableIdInclusive, a bounty creator can specify that they do not have

      // a preference for earliestAcceptableIdInclusive.
        ...
        // By submitting uint128_max as the latestAcceptableIdInclusive, a bounty creator can specify that they do not have

      // a preference for latestAcceptableIdInclusive.
      ...
      kittyCore.transferFrom(ownerOfCatBeingUsedToFulfillBounty, bounty.bidder, _kittyId);
        ...
        uint256 successfulBountyFeeInTBTCWei = totalValueIncludingFeesPerCat.sub(uint256(bounty.bountyPricePerCat));

        if(_referrer != address(0)){

            uint256 halfOfFees = successfulBountyFeeInTBTCWei.div(uint256(2));
            ...
            addressToFeeEarnings[owner()] = addressToFeeEarnings[owner()].add(successfulBountyFeeInTBTCWei);

        }



        require(ERC20(tBTCAddress).transfer(ownerOfCatBeingUsedToFulfillBounty, uint256(bounty.bountyPricePerCat)) == true, 'failed to transfer TBTC');
    ...
    ///  creator's original deposit, specified by unsuccessfulBountyFeeInTBTCWei.
    ...
    function withdrawUnsuccessfulBounty(uint256 _bountyId, address _referrer) external whenNotPaused nonReentrant {
      ...
      require(block.number >= uint256(bounty.minBlockBountyValidUntil), 'this bounty is not withdrawable until the minimum number of blocks that were originally specified have passed');
        ...
        if(totalValueRemainingForBountyId < bounty.unsuccessfulBountyFeeInTBTCWei){
            ...
            amountToTakeAsFees = bounty.unsuccessfulBountyFeeInTBTCWei;
        ...
        if(amountToReturnToBountyCreator > 0){
            ...
            require(ERC20(tBTCAddress).transfer(bounty.bidder, amountToReturnToBountyCreator) == true, 'failed to transfer TBTC');
    ...
    function withdrawBountyWithNoFeesTakenIfContractIsFrozen(uint256 _bountyId) external whenPaused nonReentrant {
      ...
      require(msg.sender == bounty.bidder || isOwner(), 'you are not the bounty creator or the contract owner');
        ...
        require(ERC20(tBTCAddress).transfer(bounty.bidder, totalValueRemainingForBountyId) == true, 'failed to transfer TBTC');
