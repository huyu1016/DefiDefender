...
interface IProtocol {
    ...
    function getToken(IERC20 token) external pure returns(IERC20);
...
contract ProtocolCompound is IProtocol, CompoundUtils {
    ...
    function getToken(IERC20 token) external pure returns(IERC20) {
        ...
        return IERC20(address(_getCToken(token)));
...
contract Holder is HolderBase {
    ...
    function moveInCallback(
        ...
        protocol.getToken(collateral).universalTransferFrom(user, address(this), protocol.getToken(collateral).balanceOf(user));
