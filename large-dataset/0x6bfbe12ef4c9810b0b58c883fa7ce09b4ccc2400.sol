...
But, BEANIE IS GIVNG BACK 20% of all the sales directly from the contract to random minters! 
...
contract BeanieGivesBack is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string baseURI;
  string public baseExtension           = ".json";
  uint256 public discountSupply         = 4;
  uint256 public costDiscount           = 0 ether;
  uint256 public costNormal             = 0.02 ether;
  uint256 public maxSupply              = 4269;
  bool    public paused                 = false;  

  constructor(
    string memory _initBaseURI
  ) ERC721("BeanieGivesBack", "BGB") {
  ...
  function mint() public payable {
    ...
    require(supply + 1 <= maxSupply, "Max supply reached!");
     ...
     if(supply < discountSupply){
          require(msg.value >= costDiscount);
        }
        else{
          require(msg.value >= costNormal);
        }
