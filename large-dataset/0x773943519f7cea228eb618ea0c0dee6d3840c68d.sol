...
contract LoihiRoot is DSMath {
    ...
    address constant liquidity = address(0);
...
contract LoihiExchange is LoihiRoot, LoihiDelegators {
    ...
    function getTargetAmount (address _oRsrv, address _tRsrv, uint256 _oNAmt, uint256[] memory _balances, uint256 _grossLiq) internal returns (uint256 tNAmt_) {
        ...
        if (omega > _psi) tNAmt_ = add(_oNFAmt, wmul(lambda, sub(omega, _psi)));

        omega = _psi;

        for (uint i = 0; i < _balances.length; i++) {
            if (reserves[i] == _oRsrv) require(_balances[i] + _oNAmt < wmul(wmul(_nGLiq, weights[i]), WAD + alpha), "origin halt check");
            if (reserves[i] == _tRsrv) require(_balances[i] - tNAmt_ > wmul(wmul(_nGLiq, weights[i]), WAD - alpha), "target halt check");
        }
    ...
    function getOriginAmount (address _oRsrv, address _tRsrv, uint256 _tNAmt, uint256[] memory _balances, uint256 _grossLiq) internal returns (uint256 oNAmt_) {
        ...
        if (omega > _psi) oNAmt_ = sub(_tNFAmt, wmul(lambda, sub(omega, _psi)));

        for (uint i = 0; i < _balances.length; i++) {
            if (reserves[i] == _oRsrv) require(_balances[i] + oNAmt_ < wmul(wmul(_nGLiq, weights[i]), WAD + alpha), "origin halt check");
            if (reserves[i] == _tRsrv) require(_balances[i] - _tNAmt > wmul(wmul(_nGLiq, weights[i]), WAD - alpha), "target halt check");
        }
