...
contract GovernorAlpha {
    ...
    string public constant name = "$DEGEN Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(degen.totalSupply(), 4), 100); } // 4% of $DEGEN

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(degen.totalSupply(), 100); } // 1% of $DEGEN
    ...
    constructor(address timelock_, address degen_) public {
        timelock = TimelockInterface(timelock_);
        degen = DegenInterface(degen_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(degen.balanceOf(msg.sender) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || degen.balanceOf(proposal.proposer) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = degen.balanceOf(voter);
