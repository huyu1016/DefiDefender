...
contract LOKPay is Membership {
    event Pay(address token, uint256 code, uint256 value);
    ...
    function pay(uint256 code) public payable {
        emit Pay(address(0), code, msg.value);
    }
    ...
    function withdraw() public {
        ...
        require(_msgSender() == receiver || isOwner(), "Incorrect address");
        receiver.transfer(address(this).balance);
    }
    
    function payBy(address token, uint256 code, uint256 value) public {
        emit Pay(token, code, value);
        IERC20(token).transferFrom(_msgSender(), tokenReceiver, value);
    }
}
