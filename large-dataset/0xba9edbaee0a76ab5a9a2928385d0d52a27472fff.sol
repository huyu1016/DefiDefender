...
contract TokenSale is Owned{
    ...
    uint256 public priceUSDT = 20;     //20c ,, made 20/100 on line 90

    uint256 public decimals = 18;

    

    IERC20Token public USDT = IERC20Token(0xdAC17F958D2ee523a2206206994597C13D831ec7);
    ...
    function setPriceUSDT(uint price_) external onlyOwner{
    ...
    function buyTokensWithETH(address _receiver) public payable { 
        ...
        require(owner.send(_amount), "Unable to transfer token to user");
    ...
    function buyTokensWithUSDT(uint _usdt, address _receiver) public { 

        uint _amount = _usdt; 

        require(_receiver != address(0), "Can't send to 0x00 address"); 

        require(_amount > 0, "Can't buy with 0 usdt"); 

        

        uint256 bal_ = USDT.balanceOf(msg.sender); //uint256

        uint256 amount_out = _amount.mul(10**6);

        require(bal_ >= amount_out, 'Balance is insuficient for this transaction'); //ok

        

        

        //require(IERC20Token(USDT).transferFrom(msg.sender, owner, amount_out), "Tokens cannot be transferred from funder account");

        USDT.safeTransferFrom(msg.sender, address(owner), amount_out);

        

        uint tokensToBuy = _amount.div(priceUSDT.div(100));
