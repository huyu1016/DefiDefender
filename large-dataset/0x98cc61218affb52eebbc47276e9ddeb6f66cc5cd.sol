...
contract DAI_Compound_Adapter is ISaffronAdapter {
  ...
  ISaffronPool internal saffron_pool;           // SaffronPool that owns this adapter

  IERC20 internal DAI;                          // Base asset (DAI)

  ICErc20 internal cDAI;                        // cDAI redeemable for DAI
  ...
  constructor(address _saffron_pool, address CErc20_contract_address, address base_asset_address) public {
  ...
  event ReturnCapital(uint256 cdai_balance, uint256 base_asset_amount, uint256 ctokens_redeemed, uint256 exchange_rate, uint256 redeem_result);
  ...
  function return_capital(uint256 base_asset_amount, address to) external override {
    ...
    uint256 ctokens_redeemable = base_asset_amount.mul(10**28).div(exchange_rate); // TODO: check significant digits
    ...
    emit ReturnCapital(cDAI.balanceOf(address(this)), base_asset_amount, ctokens_redeemable, exchange_rate, redeem_result);
  ...
  function get_holdings() external override returns(uint256) {
    ...
    return cDAI.balanceOf(address(this)).mul(cDAI.exchangeRateCurrent()).div(10**18);
