...
contract PiRatGame is IPiRatGame, Ownable, ReentrancyGuard, Pausable {
    ...
    function addToWhitelist(address[] calldata addressesToAdd) public onlyOwner {
        for (uint256 i = 0; i < addressesToAdd.length; i++) {
            _whitelistAddresses[addressesToAdd[i]] = Whitelist(true, 0, true);
        }
    }
    ...
    function claimLegendGift(address addr) public {
        require(_whitelistAddresses[addr].isWhitelisted, "Not on Legend list");
        require(_whitelistAddresses[addr].giftAvailable, "Gift already claimed");
        booty.claimGift(addr);
        _whitelistAddresses[addr].giftAvailable = false;
    }
    ...
    function showGiftAvailable(address owner) public view returns (bool) {
        return _whitelistAddresses[owner].giftAvailable;
    }
