...
contract XChanger is Ownable, XTrinity, splitConsts {
    ...
    function swapSplit(
        ...
        (uint256 returnAmount, address[] memory path, uint[] memory distribution) = quote(
        ...
        if (slipProtect) {
            ...
            uint256 feeSlippage = returnAmount.mul(slippageFee).div(PC_DENOMINATOR);    
        ...
        require (_toToken.balanceOf(address(this)) > minAmount, 'ONESPLIT slippage is too high');
    ...
    function swap(
        ...
        if (useExchange == Exchange.ONESPLIT) {
            ...
            result = XTrinity.swap(_fromToken, _toToken, amount, slipProtect);
    ...
    function quote(
    ...
    ) public view returns (uint256, address[] memory, uint[] memory) {
        ...
        if (fromToken == toToken) {
            ...
            return (amount, returnPath, distribution);
        ...
        if (useExchange == Exchange.ONESPLIT) {
            ...
            (returnAmount, distribution) = oneSplit.getExpectedReturn(_fromToken, _toToken, amount, oneSplitParts, oneSplitFlags);

            return (returnAmount, returnPath, distribution);

        } else {

            (returnAmount, , , ) = 

            XTrinity.quote(_fromToken, _toToken, returnAmount); 
