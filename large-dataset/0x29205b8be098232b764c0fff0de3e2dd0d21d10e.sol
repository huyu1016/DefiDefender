...
// Modified to work in the SOLARITE system



// all votes work on underlying _solariteBalances[address], not balanceOf(address)
...
//    Since SOLARITE supply changes, updated quorum & proposal requirements

//    If any uint96, changed to uint256 to match SOLARITE as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "SOLARITE Governor Alpha";



    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed

    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(solarite.initSupply(), 4), 100); } // 4% of SOLARITE



    /// @notice The number of votes required in order for a voter to become a proposer

    function proposalThreshold() public view returns (uint256) { return SafeMath.div(solarite.initSupply(), 100); } // 1% of SOLARITE
    ...
    SOLARITEInterface public solarite;
    ...
    constructor(address timelock_, address solarite_) public {

        timelock = TimelockInterface(timelock_);

        solarite = SOLARITEInterface(solarite_);
...

    {
        ...
        require(solarite.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
...

    {
        ...
        require(msg.sender == guardian || solarite.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
...

    {
        ...
        uint256 votes = solarite.getPriorVotes(voter, proposal.startBlock);
