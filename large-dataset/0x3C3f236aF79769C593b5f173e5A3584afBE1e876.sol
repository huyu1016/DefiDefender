...
// File hardhat/console.sol@v2.8.2
...
// File contracts/utils/Tiered.sol
...
contract Tiered is Delegated {
    struct Tier {
        uint256 price;
        uint256 supply;
        uint256 maxSupply;
        uint256 airdropSupply;
        uint256 airdropSupplyMax;
    }
    ...
    function configTiers(Tier[] memory _tierConfig) public onlyDelegates {
        for (uint256 i; i < _tierConfig.length; i++) {
            if (tierConfig.length >= i + 1) {
                require(
                    _tierConfig[i].maxSupply >= tierConfig[i].supply,
                    "Specified max supply is lower than current balance"
                );
                require(
                    _tierConfig[i].maxSupply != tierConfig[i].maxSupply,
                    "Specified max supply is equal to current max supply"
                );
                require(
                    _tierConfig[i].airdropSupplyMax >=
                        tierConfig[i].airdropSupply,
                    "Specified max airdrop supply is lower than current balance"
                );
                require(
                    _tierConfig[i].airdropSupplyMax !=
                        tierConfig[i].airdropSupplyMax,
                    "Specified max airdrop supply is equal to current max airdrop supply"
                );
                require(
                    _tierConfig[i].price != tierConfig[i].price,
                    "Specified price is equal to current price"
                );

                tierConfig[i] = Tier(
                    _tierConfig[i].price,
                    tierConfig[i].supply,
                    _tierConfig[i].maxSupply,
                    tierConfig[i].airdropSupply,
                    _tierConfig[i].airdropSupplyMax
                );
            } else {
                tierConfig.push(
                    Tier(
                        _tierConfig[i].price,
                        _tierConfig[i].supply,
                        _tierConfig[i].maxSupply,
                        _tierConfig[i].airdropSupply,
                        _tierConfig[i].airdropSupplyMax
                    )
                );
            }
    ...
    function getTierConfig() public view returns (Tier[] memory) {
        return tierConfig;
    }
}
...
// File @openzeppelin/contracts/proxy/utils/Initializable.sol@v4.4.2
...
// File contracts/utils/Juggernauts.sol
...
interface INamelessTokenData {
    function initialize(
        address templateLibrary,
        address clonableTokenAddress,
        address initialAdmin,
        uint256 maxGenerationSize
    ) external;
    ...
    Initializable
{
    ...
    function transferLegacyOwnership(address newOwner)
        public
        ...
        returns (string memory)
    {
        ...
        return
            INamelessTokenData(tokenDataContract).getTokenURI(
                tokenId,
                ownerOf(tokenId)
            );
    }
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        if (
            INamelessTokenData(tokenDataContract).beforeTokenTransfer(
                from,
                to,
                tokenId
            )
...
// File @openzeppelin/contracts/interfaces/IERC721.sol@v4.4.2

// OpenZeppelin Contracts v4.4.1 (interfaces/IERC721.sol)

pragma solidity ^0.8.0;

// File @openzeppelin/contracts/interfaces/IERC721Metadata.sol@v4.4.2

// OpenZeppelin Contracts v4.4.1 (interfaces/IERC721Metadata.sol)

pragma solidity ^0.8.0;

// File @openzeppelin/contracts/interfaces/IERC721Receiver.sol@v4.4.2
...
// File @openzeppelin/contracts/interfaces/IERC721Enumerable.sol@v4.4.2
...
// File contracts/utils/GDXERC721Batch.sol
...
// File contracts/PersonalCornerTemplate.sol
...
// File contracts/PersonalCornerSale.sol
...
contract PersonalCornerSale is Signed, Tiered {
    using Strings for uint256;

    PersonalCornerTemplate baseContract;
    NamelessToken juggernauts;
    ...
    address private a1 = 0x5d6c07fF09E0Ac7017d687474B5F739A05434b90;
    address private a2 = 0xB7edf3Cbb58ecb74BdE6298294c7AAb339F3cE4a;

    address[3] addresses = [a1, a2];
    uint256[] splits = [6, 4, 90];

    mapping(address => uint256[5]) mintMap; // Mapping of tiers minted per address

    constructor(
        Tier[] memory tiers,
        address payable baseAddress,
        address payable juggernautsAddress,
        address payable athleteAddress
    ) {
        configTiers(tiers);
        baseContract = PersonalCornerTemplate(baseAddress);
        juggernauts = NamelessToken(juggernautsAddress);
        addresses[2] = athleteAddress;
    }
    ...
    function mint(
        ...
        HolderTiers memory holderTiers = getHolderTiers(msg.sender);
        uint256 price = getPrice(holderTiers, tierId);

        require(msg.value >= quantity * price, "Ether sent is not correct");

        require(
            tierConfig[tierId].supply + quantity <=
                tierConfig[tierId].maxSupply,
        ...
        if (isMainsaleActive) {
            ...
            require(quantity == 1, "Order too big");
            require(
                mintMap[msg.sender][tierId] < 1,
                "Cannot mint more of this tier"
            );
            mintMap[msg.sender][tierId] = quantity;
        ...
        uint256 supply = baseContract.totalSupply();
        unchecked {
            for (uint256 i; i < quantity; i++) {
                baseContract.mintTier(msg.sender, supply++, tierId);
            }
        }
    }
    ...
    function mintTo(
        ...
        uint256[] calldata tierList,
        address[] calldata recipient
    ) external payable onlyDelegates {
        require(
            quantityList.length == recipient.length &&
                quantityList.length == tierList.length,
            "Must provide equal quantities, tiers, and recipients"
        );

        uint256 supply = baseContract.totalSupply();
        for (uint256 j; j < quantityList.length; j++) {
            uint256 tierIndex = tierList[j];
            require(
                tierConfig[tierIndex].supply + quantityList[j] <=
                    tierConfig[tierIndex].maxSupply,
                "Mint/order exceeds supply"
            );

            tierConfig[tierIndex].supply += quantityList[j];

            for (uint256 k; k < quantityList[j]; k++) {
                baseContract.mintTier(recipient[j], supply++, tierIndex);
            }
        }
    }
    ...
    // Jugs - 0x4ae67D3817AFd3c7990293de8d0D505B4eA29932
    function getHoldings(address _addr)
        internal
        view
        returns (uint256[] memory)
    {
        uint256 bal = juggernauts.balanceOf(_addr);
        uint256[] memory holdings = new uint256[](bal);
        if (bal > 0) {
            for (uint256 i; i < bal; i++) {
                uint256 tokenId = juggernauts.tokenOfOwnerByIndex(_addr, i);
                holdings[i] = tokenId;
            }
        }
    ...
    function hasPresaleAccess(address _addr) public view returns (bool) {
        return getHoldings(_addr).length > 0;
    }

    function getHolderTiers(address _addr)
        public
        view
        returns (HolderTiers memory)
    {
        HolderTiers memory result = HolderTiers(false, false, false, false);

        uint256[2] memory goatRange = [uint256(1), uint256(88)];
        uint256[2] memory hofRange = [uint256(100089), uint256(100976)];
        uint256[2] memory veteranRange = [uint256(100977), uint256(104528)];
        uint256[2] memory rookieRange = [uint256(104529), uint256(108888)];

        uint256[] memory holdings = getHoldings(_addr);

        for (uint256 i; i < holdings.length; i++) {
            if (holdings[i] <= goatRange[1]) {
                result.goat = true;
                continue;
            }

            if (holdings[i] >= hofRange[0] && holdings[i] <= hofRange[1]) {
                result.hof = true;
                continue;
            }

            if (
                holdings[i] >= veteranRange[0] && holdings[i] <= veteranRange[1]
            ) {
                result.veteran = true;
                continue;
            }
            ...
            if (
                holdings[i] >= rookieRange[0] && holdings[i] <= rookieRange[1]
            ) {
                result.rookie = true;
                continue;
            }
    ...
    function getPriceMult(HolderTiers memory _holderTiers)
        public
        pure
        returns (uint256)
    {
        if (_holderTiers.goat || _holderTiers.hof) {
            return 5000;
        }

        if (_holderTiers.veteran) {
            return 7500;
        }
    ...
    function getPrice(HolderTiers memory _holderTiers, uint256 tierId)
        public
        view
        returns (uint256)
    {
        uint256 price = tierConfig[tierId].price;

        if (tierId != 4 && isPresaleActive) {
            uint256 priceMult = getPriceMult(_holderTiers);
            price = (tierConfig[tierId].price * priceMult) / uint256(10000);
        }
