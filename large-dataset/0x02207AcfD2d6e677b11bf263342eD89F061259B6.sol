...
contract GoatTradersNFT is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x618fb1c684f5287f2EBDB90618b5Eefd07cC1694);

    uint256 private GOAT_MAX;
    uint256 private PRESALE_PRICE = 0.05 ether;
    uint256 private PUBLIC_PRICE = 0.05 ether;
    uint256 private REVEAL_DELAY = 0 hours;
    uint256 private PRESALE_HOURS = 0 hours;
    uint256 private PURCHASE_LIMIT = 20;
    ...
    Counters.Counter private _publicGOAT;

    constructor() ERC721("GOAT Traders NFT", "GOAT") {}
    ...
    function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicGOAT.current();

                if (_publicGOAT.current() < GOAT_MAX) {
                    _publicGOAT.increment();
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicGOAT.current() < GOAT_MAX,"Purchase would exceed GOAT_MAX");
