...
contract SocietyPunks is ERC721, Ownable, Mintable {
    string private _currentBaseURI;
    address public _owner;

    uint public tokenPrice = 0.1 ether;
    uint public tokenPrice_Presale = 0.05 ether;
    uint public max_Supply = 11170 - 125;
    ...
    bool public pause_presale = true;
    mapping(address => bool) private presaleList;   

    event bought(address buyer, uint _from, uint _to);
    ...
    function set_presalePrice(uint256 price) external onlyOwner{
        tokenPrice_Presale = price;
    }
    function set_Price(uint256 price) external onlyOwner{
        tokenPrice = price;
    }
    function set_start_time(uint256 time) external onlyOwner{
        sale_startTime = time;
    }
    function set_prestart_time(uint256 time) external onlyOwner{
    ...
    function buy(uint _count) public payable{
        ...
        emit bought(msg.sender, bought_for_imx, bought_for_imx += _count);
        bought_for_imx += _count; //Counting how many have been bought

    } 

    function buyPresale(uint _count) public payable{
        require(_count > 0, "mint at least one token");
        require(_count <= 20, "Max 20 Allowed.");
        require(bought_for_imx + _count <= max_Supply, "Not enough tokens left");
        require(msg.value == tokenPrice_Presale * _count, "incorrect ether amount");
        require(block.timestamp >= presale_startTime,"Presale not Started Yet.");
        require(pause_presale == false, "Presale is Paused.");
    ...
    function setPausePresale(bool temp) external onlyOwner {
        pause_presale = temp;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(_owner).transfer(_balance * 90 / 100); //Owner
        payable(0x9c343b8952FA16b608514A319b3705590b0f6EaB).transfer(_balance * 10 / 100); //Dev
    }
}
