...
contract Jibbydao is EIP712, Ownable, ERC721A {
    string private constant SIGNING_DOMAIN = "WEB3CLUB";
    string private constant SIGNATURE_VERSION = "1";
    uint public PRESALE_Price = 0.5 ether;

    uint public maxSupply = 1000;

    uint public presale_startTime = 1646049600; //Monday, February 28, 2022 12:00:00 PM GMT

    uint public second_presale_startTime = 1646223611; //Wednesday, March 2, 2022 12:20:11 PM GMT
    uint public second_presale_endTime = 1646136000; //Tuesday, March 1, 2022 12:00:00 PM GMT

    address private whitelistVerify = 0xbF3e689B25F460F695FC5a6715aA9c74de79e52F;

    bool public pause_sale = false;
    address public dev_address = 0x1D67bCd327093505A9b7D64A360d7297CF00c4c3;

    string public baseURI;

    mapping(address => uint) public first_presale_mint_count;
    mapping(address => uint) public second_presale_mint_count;
    ...
    DutchAuctionConfig public dutchAuctionConfig = DutchAuctionConfig({
      txLimit: 2,
      startTime: 1646136000, // Tuesday, March 1, 2022 12:00:00 PM GMT
      bottomTime: 1646140300, // Tuesday, March 1, 2022 1:11:40 PM GMT
      stepInterval: 300, // 5 minute
      startPrice: 1.5 ether,
      bottomPrice: 0.2 ether,
      priceStep: 0.1 ether
    });
    constructor() ERC721A("Jibbydao", "JBD") EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION){}

   function buy(uint _count) external payable{
        ...
        require(publicsale_mint_count[msg.sender] + _count <= 2, "Mint limit exceeded.");   
   ...
   function buy_Presale_first(uint _count, uint id, bytes memory signature) public payable{
        require(_count > 0, "mint at least one token");
        require(_count <= 2, "Max 2 Allowed in 1 Transaction.");
        require(totalSupply() + _count < maxSupply, "Max Supply Reached.");
        ...
        require(block.timestamp < second_presale_startTime,"Presale Ended.");
        require(pause_sale == false, "Sale is Paused.");
        require(check(id,signature) == whitelistVerify, "You are not whitelisted.");
        require(first_presale_mint_count[msg.sender] + _count <= 2, "Mint limit exceeded."); 
        require(voucherIds[id] == 0, "Invalid Voucher."); //To verify that same voucher id is not being used again.
       

            _safeMint(msg.sender, _count);
            
            first_presale_mint_count[msg.sender] += _count;
            voucherIds[id] = 1;
    }

   function buy_Presale_second(uint _count, uint id, bytes memory signature) public payable{
        require(_count > 0, "mint at least one token");
        require(_count <= 1, "Max 1 Allowed in 1 Transaction.");
        require(totalSupply() + _count < maxSupply, "Max Supply Reached.");
        require(msg.value == PRESALE_Price * _count, "incorrect ether amount");
        require(block.timestamp >= second_presale_startTime,"Presale have not started yet.");
        require(block.timestamp < second_presale_endTime,"Presale Ended.");
        require(pause_sale == false, "Sale is Paused.");
        require(check(id,signature) == whitelistVerify, "You are not whitelisted.");
        require(second_presale_mint_count[msg.sender] + _count <= 1, "One address can't mint more than total 1 in presale.");        
        require(voucherIds[id] == 0, "Invalid Voucher."); //To verify that same voucher id is not being used again.

            _safeMint(msg.sender, _count);
            
            second_presale_mint_count[msg.sender] += _count;
            voucherIds[id] = 1;
    }
  ...
  function configureDutchAuction(
    ...
    function set_second_presale_start_time(uint _starttime, uint _endtime) external onlyOwner{
        second_presale_startTime = _starttime;
        second_presale_endTime = _endtime;
    }    
    ...
    function withdraw() external onlyOwner {
        ...
        payable(dev_address).transfer(_balance * 15 / 100); //dev
        payable(msg.sender).transfer(_balance * 85 / 100); //Owner
    }
