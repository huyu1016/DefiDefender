/**
 * Source Code first verified at https://etherscan.io on Tuesday, October 23, 2018
 (UTC) */
...
//功能列表：
//1.控制公开销售时，基石和VC的销售额度
//2.未售出，则直接销毁(销毁的Token从YOUSale出)
//3.添加操作事件
//4.理清销售合约和代币合约关系(Sale合约持有Token合约)
//5.需要解决一个账户只能购买一次问题(引入期权概念)
//6.如何分配未未公开发行的token(直接转账)，锁定期问题(由YOUSale分配
//7.添加查看接口
//8.客户可以方便的自己解冻token(token合约调用clim方法)
//9.可以暂停交易
