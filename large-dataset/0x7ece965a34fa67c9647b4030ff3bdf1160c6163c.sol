...
interface NFTFactoryInterface {
    ...
    function getHashPrice(uint256 tokenId) external view returns(uint256);
...
contract LPStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(
        ...
        pair = IUniswapV2Pair(_stakingToken);
    ...
    function getReserves() internal view returns (uint, uint) {
        ...
        (uint reserves1, uint reserves2,) = pair.getReserves();
    ...
    function getPrice() public view returns (uint) {
        ...
        //ETH pool amount / balance LP * 2

        (uint amount,,) = pair.getReserves();

        uint price = amount * 1e18 / pair.totalSupply() * 2;
    ...
    function getAmountsIn(uint amount, address token) internal view returns (uint) {

        uint[] memory amounts = uniswap.getAmountsIn(amount, getPathForTokenToEth(token));

        uint256 outputTokenCount = uint256(amounts[0]);
    ...
    function swapExactETHForTokens(uint amountEth) internal {
        ...
        uint[] memory amounts = uniswap.swapExactETHForTokens.value(amountEth)(0, getPathForTokenToEth(address(NDR)), address(this), now + deadline);
    ...
    function addLiquidityETH(uint amountTokenDesired, uint amountEth) internal {
        ...
        uniswap.addLiquidityETH.value(amountEth)(address(NDR), amountTokenDesired, amountTokenDesired * 9 / 10, amountEth * 9 / 10, address(0), now + deadline);
    ...
    function changeMultiplicator(uint256 _mul1, uint256 _mul2) public onlyOwner returns(uint256, uint256) {
    ...
    function changeInterest(uint256 newInterest) public onlyOwner returns(uint256) {
...

    {
        ...
        require(amount >= minLp && amount <= maxLp, "wrong amount");
...

    {
        ...
        uint256 amount = NFTFactoryInterface(address(NTFToken)).getHashPrice(tokenId); //nft price in HASH

        require(amount > 0, "can't buy in HASH");

        require(reward >= amount, "not enough HASH");
    ...
    function buy(uint256 tokenId) public payable {
        ...
        uint256 amount = NFTFactoryInterface(address(NTFToken)).getRarity(tokenId);

        uint256 price = getPrice() * amount * mul1 / mul2;
        ...
        addLiquidityETH(amountToken, amountEth * 9 / 10);
