...
contract ChordusArena is ERC721Enumerable, Ownable{
    ...
    uint public constant MAX_NFT_PURCHASE_PRESALE = 10;
    uint public MAX_SUPPLY = 5000;
    bool public saleIsActive = false;
    bool public presaleIsActive = false;
    bool public isMetadataLocked = false; // Base URL is frozen if true
    uint public nftReserve = 150;
    uint public plusOneDiscountQuantity = 4;
    ...
    constructor() ERC721("Chordus Arena","CHORDUSARENA"){
    }

    function setPlusOneDiscountQuantity(uint count) public onlyOwner {
        require(0 < count, "plusOneDiscountQuantity Can't be 0");
        plusOneDiscountQuantity = count;
    }
    ...
    function mint(uint numberOfTokens) public payable {
        ...
        require( (totalSupply() + nftReserve + numberOfTokens) <= (MAX_SUPPLY), "Purchase would exceed max supply of NFTs");
        ...
        if (numberOfTokens == plusOneDiscountQuantity) {
            numberOfTokens++;
        }
    ...
    function _mintPresale(uint numberOfTokens) internal {
        require(presaleIsActive, "Presale is not active at the moment");
        require(numberOfTokens > 0, "Number of tokens can not be less than or equal to 0");
        require( (totalSupply() + nftReserve + numberOfTokens) <= (MAX_SUPPLY), "Purchase would exceed max supply of NFTs");
        require(_whiteListClaimed[msg.sender] + numberOfTokens <= MAX_NFT_PURCHASE_PRESALE,"Exceeds supply of presale NFTs you can mint.");
        require( (NFT_PRICE * numberOfTokens) <= msg.value, "Sent ether value is incorrect");
   ...
   function mintWhitelist(
       ...
       bytes32[] calldata proof
        ...
        require(_verify(_leafNode, proof), "Invalid proof");
        _mintPresale(numberOfTokens);
    }
