...
contract Taiketsu is ERC721A, ERC721Holder, Ownable{
    ...
    uint public price;
    uint public priceWL;
    uint public reservedForMG;
    uint public totalSwapped;

    mapping (address => uint) private whitelist;
    uint private whitelistCount = 5;

    MG public constant MintGame = MG(0x7EF68725E9e971B825699D667034Dd482FAe372a);

    constructor() ERC721A("Taiketsu", "TKTS", MAX_TOKENS_PER_TX, MAX_TOKENS)  {
        
    }

    function mint(address _to, uint _count) public payable {
      require(totalSupply() + _count <= MAX_TOKENS, "Exceeds limit");
      require(totalSupply() + _count <= MAX_TOKENS - reservedForMG + totalSwapped, "Exceeds limit for sale");
    ...
    function swapMintGamers(uint[] memory _ids) public {
      require(totalSupply() + _ids.length <= MAX_TOKENS, "Exceeds limit");
      require(totalSwapped + _ids.length <= reservedForMG, "Exceeds limit for swap");
      require(_ids.length <= MAX_TOKENS_PER_TX, "Exceeds TX limit");
      for(uint i = 0; i < _ids.length; i++){
        require(MintGame.ownerOf(_ids[i]) == msg.sender, "Sender not owner");
        MintGame.safeTransferFrom(msg.sender, owner(), _ids[i]);
      }
      uint _count = _ids.length;
      totalSwapped += _count;
    ...
    function setReservedForMG(uint _reserved) public onlyOwner {
        reservedForMG = _reserved;
    }
...
interface MG{
    ...
    function getTokenSkill(uint id) external view returns (uint256);
}
