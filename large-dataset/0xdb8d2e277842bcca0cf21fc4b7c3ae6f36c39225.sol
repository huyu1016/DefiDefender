...
contract WhalePunkz is ERC721, Ownable {
    ...
    uint256 public publicTokenPrice = 0.01 ether;
	uint public maxPerTx = 20;
	uint public maxFreeMintPerWallet = 10;
	uint public amountAvailableFreeMint = 1111;
    ...
    constructor(address payable shareholderAddress_) ERC721("Whale Punkz", "WHALE") {
    ...
    function mint(uint numberOfTokens) external payable {
        ...
        require(numberOfTokens <= maxPerTx, "You can only mint 20 during the main sale and 10 during the free.");
        ...
        if (_tokenSupply.current() + numberOfTokens > amountAvailableFreeMint) {
            require((publicTokenPrice * numberOfTokens) == msg.value, "Send the exact amount in ETH (0.01 per token).");
        } else {
			require(msg.value == 0, "Don't send ether for the free mint.");
			require(addressFreeMinted[msg.sender] < maxFreeMintPerWallet, "The free sale is limited to 10 whales per wallet. Public is limited to 20 per tx.");
		}
