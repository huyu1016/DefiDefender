...
// Modified to work in the GRAP system
// all votes work on underlying _grapBalances[address], not balanceOf(address)
...
//    Since GRAP supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match GRAP as opposed to comp
contract GovernorAlpha {
    /// @notice The name of this contract
    string public constant name = "GRAP Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(grap.initSupply(), 4), 100); } // 4% of GRAP

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(grap.initSupply(), 100); } // 1% of GRAP
    ...
    constructor(address timelock_, address grap_) public {
        timelock = TimelockInterface(timelock_);
        grap = GRAPInterface(grap_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(grap.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || grap.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = grap.getPriorVotes(voter, proposal.startBlock);
