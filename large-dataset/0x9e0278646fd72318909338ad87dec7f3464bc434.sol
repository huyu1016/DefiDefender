...
contract SaffronStrategy is ISaffronStrategy {
  ...
  uint256[] public pool_SFI_rewards = [200, 410, 210, 100, 210, 10, 50, 410, 50, 200, 25];
  ...
  function wind_down_epoch(uint256 epoch) external {
    ...
    require(epoch == 4, "v1.4: only epoch 4");
    ...
    uint256 rariDaiSFI = (200 * 1 ether);

    SFI(SFI_address).mint_SFI(pools[9], rariDaiSFI);

    ISaffronPool(pools[9]).wind_down_epoch(epoch, rariDaiSFI);



    // SFI/PRT pool (25 SFI)

    uint256 prtSFI = (25 * 1 ether);

    SFI(SFI_address).mint_SFI(pools[10], prtSFI);

    ISaffronPool(pools[10]).wind_down_epoch(epoch, prtSFI);
  ...
  function wind_down_pool(uint256 pool, uint256 epoch) external {
    ...
    if (pool == 1) { // SFI/ETH Uniswap LP Pool (410 SFI)
    ...
    if (pool == 2) { // SFI Staking Pool (210 SFI)
    ...
    if (pool == 3) { // SFI/BTSE Uniswap LP Pool (100 SFI)
    ...
    if (pool == 4) { // rSFI Staking Pool (210 SFI)
    ...
    if (pool == 5) { // SFI/GEEQ Uniswap LP Pool (10 SFI)
    ...
    if (pool == 6) { // SFI/ESD Uniswap LP Pool (50 SFI)
    ...
    if (pool == 7) { // SFI/ETH SushiSwap LP Pool (410 SFI)
    ...
    if (pool == 8) { // ibETH/Alpha Pool (50 SFI)
    ...
    if (pool == 9) { // Rari Dai Pool (200 SFI)
    ...
    if (pool == 10) { // SFI/PRT Pool (25 SFI)
