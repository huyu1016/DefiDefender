...
contract JumpRope is Ownable {
    ...
    mapping (address => uint256) public ropeDeposit;
    ...
    constructor(Hope _hopeAddress, IERC20 _ropeAddress, uint256 _startBlock, uint256 _endBlock) public {
    ...
    function fundSale(uint256 _amount) public onlyOwner {

        require(block.number < endBlock, "Sale has ended");

        require(_amount >= 0, "Amount cannot be negative");

        hope.transferFrom(msg.sender, address(this), _amount);

        hopeTotal = hope.balanceOf(address(this));

    }



    function collectRopes() public onlyOwner {

        require(block.number > endBlock, "Sale has not ended");

        rope.transfer(msg.sender, rope.balanceOf(address(this)));
    ...
    function deposit(uint256 _amount) public {
        ...
        rope.transferFrom(msg.sender, address(this), _amount);

        ropeDeposit[msg.sender] = ropeDeposit[msg.sender].add(_amount);

        ropeTotal = ropeTotal.add(_amount);
    ...
    function collect() public {
        ...
        require(ropeDeposit[msg.sender] > 0, "Address did not contribute");



        hasCollected[msg.sender] = true;

        uint256 contribution = ropeDeposit[msg.sender].mul(1e12).div(ropeTotal);

        uint256 hopeAmount = hopeTotal.mul(contribution).div(1e12);
