...
// File contracts/token/IERC20Mintable.sol
...
// File contracts/state/StateMachine.sol
...
contract StateMachine {
    ...
    event Transition(bytes32 _currentStage, bytes32 _nextStage);
...

    {
        ...
        emit Transition(currentStage, _targetStage);
...
// File hardhat/console.sol@v2.0.4
...
contract Issuance is Ownable, ReentrancyGuard, StateMachine {
    ...
    bytes32 constant private OPEN_STAGE = "OPEN";

    bytes32 constant private CLOSE_STAGE = "CLOSE";

    bytes32 constant private WITHDRAW_STAGE = "WITHDRAW";
...

    {
        ...
        _addTransition(SETUP_STAGE, OPEN_STAGE);

        _addTransition(OPEN_STAGE, CLOSE_STAGE);

        _addTransition(CLOSE_STAGE, WITHDRAW_STAGE);

        _addTransition(WITHDRAW_STAGE, SETUP_STAGE);
    ...
    ) public onlyOwner atStage(SETUP_STAGE) {
    ...
    function setEvent(bytes32 _event) public onlyOwner {
        ...
        require(
            ...
            currentStage == SETUP_STAGE || currentStage == OPEN_STAGE,
    ...
    function invest() external payable nonReentrant atStage(OPEN_STAGE) {
        ...
        investments[msg.sender] = investments[msg.sender].add(

            _receiveAmount.add(_investInWei.mul(_saleEvent.issuancePrice))
