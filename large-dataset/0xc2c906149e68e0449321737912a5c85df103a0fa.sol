...
contract DfFinanceCloseCompound is DSMath, ConstantAddresses, FundsMgr {
    ...
    function _takeFeeHelper(
        ...
        uint256 ethRestAfterFee, bool hasProfit, uint256 extraProfitEth
        ...
        if (_ethRest > add(_strategy.deposit, userSelfLoan)) {

            extraProfitEth = sub(_ethRest, add(_strategy.deposit, userSelfLoan));
        ...
        if (hasProfit) {
            ...
            require(_ethRest >= ethAfterFee, "ethRest less then ethAfterFee");



            ethRestAfterFee = ethAfterFee;
            ...
            if (now < dateUntilFees) {
                ...
                ethRestAfterFee = sub(_ethRest, mul(_ethRest, earlyCloseFee) / 100);
