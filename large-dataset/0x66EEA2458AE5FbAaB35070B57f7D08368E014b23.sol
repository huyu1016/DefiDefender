...
contract CATA is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public presaleTokenPrice = 40000000000000000; // 0.04 ETH
    ...
    function preSaleToken( uint256[] memory _ids) public payable {
        ...
        require(presaleTokenPrice * _ids.length <= msg.value, "Ether value sent is not correct");
    ...
    function mintToken( uint256[] memory _ids) public payable {
        ...
        require(_numberOfWallets[msg.sender] + _ids.length <= maxTokenPerWallet, "Exceeded max token purchase per wallet");
        require(totalSupply() + _ids.length <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
        require(tokenPrice * _ids.length <= msg.value, "Ether value sent is not correct");
		for (uint256 i = 0; i < _ids.length; i++) {
            require(_isOccupiedId[_ids[i]] == false, "MINT: Those ids already have been used for other customers");
        }
        for(uint i = 0; i < _ids.length; i++) {
