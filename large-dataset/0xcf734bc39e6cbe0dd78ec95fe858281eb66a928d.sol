...
//////////////////////////////////////////////////
//////////
//////////  Bounty Blobs, by Ryan Lee
//////////  A decentralized refer-a-friend concept. 
//////////
//////////////////////////////////////////////////
...
// File: contracts/BountyBlobs.sol
...
contract BountyBlobs is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant MINT_PRICE   = 6 * 10**16; // 0.06 ETH
    uint256 public constant BOUNTY_PRICE = 2 * 10**16; // 0.02 ETH
...
string[] private durSecs = [
    ...
    event CreateBountyBlobs(uint256 indexed id);

    constructor() public ERC721("BountyBlobs", "BLOB") {}
    ...
    // Let the owner mint to get the bounty started
    function mintFirstForOwner(string memory _path) public payable onlyOwner {
        uint256 total = _totalSupply();
        require(total + 1 <= 2, "Max limit");
        require(total <= 2, "Sale end");
    ...
    function mint(string memory _path) public payable {
        uint256 total = _totalSupply();
        ...
        uint256 rand = uint256(keccak256(abi.encodePacked(Strings.toString(total))));
        uint256 bountyWinnerTokenId = rand % total;
        address bountyWinner = ERC721.ownerOf(bountyWinnerTokenId);
        _widthdraw(bountyWinner, BOUNTY_PRICE);
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
      ...
      parts[3] = getRandomColor(_tokenId, "BLOB COLOR ONE", blobColors);
      parts[4] = '" offset="0%"></stop><stop id="stop2" stop-color="';
      parts[5] = getRandomColor(_tokenId, "BLOB COLOR TWO", blobColors);
