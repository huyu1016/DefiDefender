...
contract NFTeGG_Farm_1 is Ownable {
    ...
    event StakeT1(address indexed user, uint256 indexed tokenid);

    event UnstakeT1(address indexed user, uint256 indexed tokenid);

    event Payout(address indexed user, uint256 value);



    uint public constant BLOCKTIME_PENALTY_THRESHOLD = 3600 * 72; // minimum time (in seconds) for staking to receive payout without penalty



    address public constant T1_ADDRESS = address(0x10a7ecF97c46e3229fE1B801a5943153762e0cF0); // address of T1 contract (ERC721) used for staking

    address public constant T3_ADDRESS = address(0xD821E91DB8aED33641ac5D122553cf10de49c8eF); // address of T3 contract (ERC20) used for distributing rewards



    uint256 public constant T1_BOOL_MUL_TRUE = 12;

    uint256 public constant T1_BOOL_MUL_FALSE = 8;

    uint256 public constant T1_BOOL_DIVI = 10;



    uint256 public t1StakeFactor; // stake payout per power and block for staked T1
    ...
    mapping (address => StakeFactorERC20) public erc20StakeFactors; // maps ERC20 token address to its stake factor
    ...
    mapping (uint256 => address) public stakedT1Holders; // maps which token id is staked by which user

    mapping (address => mapping (uint256 => StakingDataT1)) public t1Stakes; // maps staker address onto a mapping from tokenid to the struct specifying when the stake was made

    mapping(address => uint256[]) public _nftStake;
    ...
    constructor (uint256 t1StakeFactor_) public {

        t1StakeFactor = t1StakeFactor_;
    ...
    function calcCurrentPayoutT1(address user, uint256 tokenid) public view returns (uint256) {

        uint stakeBlock = t1Stakes[user][tokenid].setupBlock;

        require(stakeBlock != 0, "User did not stake that token");



        if ((block.timestamp - t1Stakes[user][tokenid].setupTime) >= BLOCKTIME_PENALTY_THRESHOLD) { // normal payout if minimum time was reached

            return (block.number - stakeBlock) * T1(T1_ADDRESS).power(tokenid) * t1StakeFactor * (T1(T1_ADDRESS).someBool(tokenid) ? T1_BOOL_MUL_TRUE : T1_BOOL_MUL_FALSE) / T1_BOOL_DIVI;

        } else { // 90% payout if minimum time was not reached

            return (block.number - stakeBlock) * T1(T1_ADDRESS).power(tokenid) * t1StakeFactor * 9 * (T1(T1_ADDRESS).someBool(tokenid) ? T1_BOOL_MUL_TRUE : T1_BOOL_MUL_FALSE) / T1_BOOL_DIVI / 10;
    ...
    function stakeT1(uint256 tokenid) external {

        address user = msg.sender;

        IERC721(T1_ADDRESS).transferFrom(user, address(this), tokenid);

        stakedT1Holders[tokenid] = user;

        t1Stakes[user][tokenid] = StakingDataT1(block.number, block.timestamp);

        uint256[] storage nftIds = _nftStake[msg.sender];
        ...
        _nftMapIndex[tokenid] = nftIds.length - 1;

        StakeT1(user, tokenid);

    }



    function unstakeT1(uint256 tokenid) external {

        address user = msg.sender;

        require(stakedT1Holders[tokenid] == user, "User did not stake the specified token");

        delete stakedT1Holders[tokenid];

        uint256 payout = accumulatedPayouts[user] + calcCurrentPayoutT1(user, tokenid);

        delete t1Stakes[user][tokenid];

        _payout(user, payout);

        T1(T1_ADDRESS).setSomeBool(tokenid, false);

        IERC721(T1_ADDRESS).transferFrom(address(this), user, tokenid);

        

        uint256[] memory gegoIds = _nftStake[msg.sender];

        uint256 nftIndex = _nftMapIndex[tokenid]; 

        uint256 gegoArrayLength = gegoIds.length-1;

        uint256 tailId = gegoIds[gegoArrayLength];

        _nftStake[msg.sender][nftIndex] = tailId;

        _nftStake[msg.sender][gegoArrayLength] = 0;

        _nftStake[msg.sender].pop();

        _nftMapIndex[tailId] = nftIndex;

        _nftMapIndex[tokenid] = 0;

        

        UnstakeT1(user, tokenid);

    }



    function payoutRewardT1(uint256 tokenid) external {
        ...
        t1Stakes[user][tokenid].setupBlock = block.number;
    ...
    function addERC20ForStaking(address erc20ContractAddress, uint128 stakeFactorMulti, uint128 stakeFactorDivi) onlyOwner external {
        ...
        erc20StakeFactors[erc20ContractAddress] = StakeFactorERC20(stakeFactorMulti, stakeFactorDivi);

    }



    function changeFactorT1(uint256 newFactor) onlyOwner external {

        t1StakeFactor = newFactor;
    ...
    function getNftIDs( address account ) public view returns( uint256[] memory tokenid )

    {

        tokenid = _nftStake[account];
