...
contract WavelengthXmfers is AbstractEditionContract  {
    ...
    function isValidClaim( uint256 numPieces,
        ...
        {
            isValid = verifyMerkleProof(merkleProof, editionIndex, amount);
            require(
                isValid,
    ...
    function verifyMerkleProof(bytes32[] calldata merkleProof, uint256 editionIndex, uint amount) public view returns (bool) {
        if(!editions[editionIndex].merkleProtect)
        {
            return true;
        }
        ...
        return MerkleProof.verify(merkleProof, editions[editionIndex].merkleRoot, node);
    }
