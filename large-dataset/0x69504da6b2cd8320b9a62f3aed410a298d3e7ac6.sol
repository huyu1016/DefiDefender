...
contract NAVFeed is BaseNFTFeed, Interest, Buckets, FixedPoint {
    ...
    function init() public {
        ...
        // risk group: 0 - ADF, APR: 11%
       
        file("riskGroup", 0, ONE, ONE, uint(1000000003488077118214104515), 99.91 * 10**25);
        // risk group: 1 - BDF, APR: 11.5%
        file("riskGroup", 1, ONE, ONE, uint(1000000003646626078132927447), 99.85 * 10**25);
        // risk group: 2 - CDF, APR: 11.5%
        file("riskGroup", 2, ONE, 95 * 10**25, uint(1000000003646626078132927447), 99.65 * 10**25);
        // risk group: 3 - DDF, APR: 12%
        file("riskGroup", 3, ONE, 95 * 10**25, uint(1000000003805175038051750380), 99.55 * 10**25);
        // risk group: 4 - ARF, APR: 11%
        file("riskGroup", 4, ONE, 95 * 10**25, uint(1000000003488077118214104515), 99.91 * 10**25);
        // risk group: 5 - BRF, APR: 11.5%
        file("riskGroup", 5, ONE, 90 * 10**25, uint(1000000003646626078132927447), 99.85 * 10**25);
        // risk group: 6 - CRF, APR: 11.5%
        file("riskGroup", 6, ONE, 80 * 10**25, uint(1000000003646626078132927447), 99.65 * 10**25);
        // risk group: 7 - DRF, APR: 12%
        file("riskGroup", 7, ONE, 70 * 10**25, uint(1000000003805175038051750380), 99.55 * 10**25);
        // risk group: 8 - ATF, APR: 11%
        file("riskGroup", 8, ONE, 80 * 10**25, uint(1000000003488077118214104515), 99.94 * 10**25);
        // risk group: 9 - BTF, APR: 11.5%
        file("riskGroup", 9, ONE, 70 * 10**25, uint(1000000003646626078132927447), 99.9 * 10**25);
        // risk group: 10 - CTF, APR: 11.5%
        file("riskGroup", 10, ONE, 60 * 10**25, uint(1000000003646626078132927447), 99.79 * 10**25);
        // risk group: 11 - DTF, APR: 12%
        file("riskGroup", 11, ONE, 50 * 10**25, uint(1000000003805175038051750380), 99.73 * 10**25);


       /// Overdue loans (= loans that were not repaid by the maturityDate) are moved to write Offs
        // write-off group: 0 - 10% write off
        setWriteOff(0, WRITE_OFF_PHASE_A, ONE, 90 * 10**25);
        // write-off group: 1 - 25% write off
        setWriteOff(1, WRITE_OFF_PHASE_B, ONE, 75 * 10**25);
