...
// File: contracts/EIP712Claimable.sol
...
contract EIP712Claimable is Ownable {
    ...
    address claimSigningKey = address(0);
    ...
    function setClaimSigningAddress(address newSigningKey) public onlyOwner {
        claimSigningKey = newSigningKey;
    }

    modifier requiresClaim(bytes calldata signature, uint256 count) {
        require(claimSigningKey != address(0), "claiming not enabled");
        ...
        require(recoveredAddress == claimSigningKey, "Invalid Signature");
        _;
    }
}

// File: contracts/floor_gen3.sol
...
contract FloorGen3 is ERC721, ReentrancyGuard, Ownable, EIP712Claimable{
  ...
  bool public claimingIsActive = true;
  ...
  function updateClaimCount(address minter, uint256 count) private {
    claimCounts[minter] += count;
  }

  function claimTokens(uint256 count, bytes calldata signature) public payable requiresClaim(signature, count) nonReentrant  {
    require(claimingIsActive, "Claiming not active");
    require(totalReservedSupply() + count - 1 < MAX_RESERVED_SUPPLY, "Exceeds max reserved supply");
    require(totalSupply() + count - 1 < MAX_SUPPLY , "Exceeds max supply");

    if (hasUnclaimedTokens(msg.sender)) {
      updateClaimCount(msg.sender, count);
    } else {
      revert("You have already claimed all your tokens");
    }
