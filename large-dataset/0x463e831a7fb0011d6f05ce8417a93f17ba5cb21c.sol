...
// File: contracts/interfaces/OneInchExchangeInterface.sol
...
contract DfFinanceMgrOpenCdp is Initializable, DSMath, FundsMgrUpgradable {
    ...
    function initialize() initializer public {
        ...
        tub = TubInterface(0x448a5065aeBB8E423F0896E6c5D525C040f59af3);
        inFee = 0;
    ...
    function setDfProxyBetAddress(DfProxyBetInterface proxyBet, uint256 newInsuranceCoef) onlyOwner public {
        require(address(proxyBet) != address(0) && insuranceCoef > 0 
            || address(proxyBet) == address(0) && insuranceCoef == 0, "Incorrect proxy address or insurance coefficient");
    ...
    function exchange(TokenInterface fromToken, uint256 maxFromTokenAmount, TokenInterface toToken, uint256 minToTokenAmount, bytes memory _data) internal returns (uint256)
    {
        ...
        uint256 amountEther = 0;
        if (fromToken != ETH_ADDRESS) {
            if (fromToken.allowance(address(this), ex.spender()) != uint256(-1)) { // TODO: optimize
                fromToken.approve(ex.spender(), uint256(-1));
            }
        } else {
            amountEther = maxFromTokenAmount;
        }
        ...
        // call(g, a, v, in, insize, out, outsize)
            let succeeded := call(sub(gas, 5000), ex, amountEther, add(_data, 0x20), mload(_data), 0, 32)
