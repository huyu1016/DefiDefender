...
contract NFTSale is Ownable
{
    ...
    uint256 public constant MAX_UNITS_PER_TRANSACTION = 10;

    uint256 public vipSaleStartTimestamp;
    uint256 public wlSaleStartTimestamp;
    ...
    address public incomeRecipientWallet;
    ...
    constructor(
        ...
        address _signaturePublicKey,
        address _incomeRecipientWallet,
        uint256 _vipSaleStartTimestamp,
        uint256 _wlSaleStartTimestamp,
        uint256 _publicSaleStartTimestamp)
    {
        
        vipSaleStartTimestamp = _vipSaleStartTimestamp;
        wlSaleStartTimestamp = _wlSaleStartTimestamp;
        publicSaleStartTimestamp = _publicSaleStartTimestamp;

        vipPrice = 0.2 ether;
        wlPrice = 0.3 ether;
        publicPrice = 0.4 ether;

        //@todo change to - 0x5aefce0a2d473f59578fa7dee6a122d6509af1e0f79fcbee700dfcfeddabe4cc
        signaturePublicKey = _signaturePublicKey;

        incomeRecipientWallet = _incomeRecipientWallet;

        royaltyDistributor = IRoyaltyDistributor(_royaltyDistributorAddress);
        token = INFT(_tokenAddress);
    }

    function buyForVIP(uint256 _amount, bytes memory _signature) external payable
    {
        if (msg.sender != owner()) {
    ...
    function buyForWL(uint256 _amount, bytes memory _signature) external payable
    {
    ...
    {
        require(publicSaleStartTimestamp <= block.timestamp, "sale not started");

        require(_amount <= MAX_UNITS_PER_TRANSACTION, "the amount is out of range");

        require(msg.value == _amount * publicPrice, "invalid value");

        _safeTransferETH(incomeRecipientWallet, address(this).balance);

        (uint256 fromId, uint256 toId) = token.mintBatch(msg.sender, _amount);
        royaltyDistributor.setRoyaltyKindForTokenBatch(fromId, toId, 0);
    }

    function getTokenForOwner(
        address _recipient,
        address _tokenAddress) external onlyOwner
    {
        IERC20 _token = IERC20(_tokenAddress);
        _token.safeTransfer(_recipient, _token.balanceOf(address(this)));
    }

    function _verifyVIP(address _sender, bytes memory _signature) private view returns (bool)
    {
        bytes32 message = _prefixed(keccak256(abi.encodePacked("VIP", _sender, address(this))));
        return _recoverSignature(message, _signature) == signaturePublicKey;
    }

    function _verifyWL(address _sender, bytes memory _signature) private view returns (bool)
    {
        bytes32 message = _prefixed(keccak256(abi.encodePacked("WL", _sender, address(this))));
        return _recoverSignature(message, _signature) == signaturePublicKey;
    }

    function _recoverSignature(
        bytes32 _message,
        bytes memory _signature) private pure returns (address)
    {
