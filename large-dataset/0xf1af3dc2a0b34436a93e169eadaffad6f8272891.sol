 ...
 * @title TendiesCrowdsale

 * @dev Crowdsale contract for $TEND. 
 ...
 *      1 ETH = 20000 TEND (during the entire sale)
...
contract TendiesCrowdsale is Ownable {
    ...
    uint256 public constant ROUND_1_CAP = 1.5 ether;

    uint256 public constant ROUND_2_CAP = 3 ether;
    ...
    // Start time (07/28/2020 @ 2:00pm (UTC))

    uint256 public constant CROWDSALE_START_TIME = 1595944800;
    ...
    uint256 public constant TEND_PER_ETH = 20000;



    // Round 1 whitelist

    mapping(address => uint256) public whitelistCapsRound1;



    // Round 2 whitelist

    mapping(address => uint256) public whitelistCapsRound2;
    ...
    constructor(IERC20 _tendToken) Ownable() public {
    ...
    function _buyTokens(address beneficiary, uint256 weiAmount) internal {
        ...
        tendToken.safeTransfer(beneficiary, tokenAmount);
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(beneficiary != address(0), "TendiesCrowdsale: beneficiary is the zero address");

        require(isOpen(), "TendiesCrowdsale: sale did not start yet.");

        require(!hasEnded(), "TendiesCrowdsale: sale is over.");

        require(weiAmount >= MIN_CONTRIBUTION, "TendiesCrowdsale: weiAmount is smaller than min contribution.");

        require(!isWithinCappedSaleWindow() ||

            (contributions[beneficiary].add(weiAmount) <= whitelistCapsRound1[beneficiary] || 

            contributions[beneficiary].add(weiAmount) <= whitelistCapsRound2[beneficiary]),

            "TendiesCrowdsale: individual cap exceeded or not whitelisted"
    ...
    function _getTokenAmount(uint256 weiAmount) internal pure returns (uint256) {
        ...
        return weiAmount.mul(TEND_PER_ETH);
    ...
    function isWithinCappedSaleWindow() public view returns (bool) {
        ...
        return now >= CROWDSALE_START_TIME && now <= (CROWDSALE_START_TIME + 1 hours);
    ...
    function setWhitelist2(address[] calldata accounts) external onlyOwner {

        for (uint256 i = 0; i < accounts.length; i++) {

            whitelistCapsRound2[accounts[i]] = ROUND_2_CAP;
    ...
    function addAndLockLiquidity() external {
        ...
        require(hasEnded(), "TendiesCrowdsale: can only send liquidity once hardcap is reached");



        uint256 amountEthForUniswap = address(this).balance;

        uint256 amountTokensForUniswap = tendToken.balanceOf(address(this));



        // Unpause transfers forever

        Pauseable(address(tendToken)).unpause();

        // Send the entire balance and all tokens in the contract to Uniswap LP

        tendToken.approve(address(uniswapRouter), amountTokensForUniswap);
