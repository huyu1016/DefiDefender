...
contract DeFi {
  ...
  function convertToSynth(address _token, uint256 _amount) private view returns(uint256) {
  ...
  function convertFromSynth(address _token, uint256 _amount) private view returns(uint256) {
  ...
  function convertEthToMet(uint256 _amount) private view returns(uint256) {
    ...
    return ((_amount.mul(1e18)).div(price_));
  ...
  function convertMetToEth(uint256 _amount) private view returns(uint256) {
    ...
    return (_amount.mul(price_)).div(1e18);
