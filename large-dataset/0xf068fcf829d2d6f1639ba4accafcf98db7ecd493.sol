...
contract TrumpyDumpy is ERC721Enumerable, Ownable {
    ...
    uint256 public constant CURRENT_PRICE = 0.08 ether;
        ...
        returns (uint256)
    {
        ...
        return addressToMinted[_msgSender()];
    }
    ...
    function giveAway(address[] calldata _user) external onlyOwner {
        uint256 totalSupply = _owners.length;
        require(totalSupply + _user.length <= MAX_SUPPLY, "Excedes max supply.");
        for (uint256 i; i < _user.length; i++) _mint(_user[i], totalSupply + i);
    }
    ...
    function publicMint(uint256 count) public payable {
        ...
        require(count * CURRENT_PRICE == msg.value, "Invalid funds provided.");
