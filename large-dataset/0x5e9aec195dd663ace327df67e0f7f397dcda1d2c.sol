...
contract StateProtocol is Context, IERC20, Ownable {
    ...
    string private _name = "State Protocol";

    string private _symbol = "STATE";
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        require(
            ...
            "State Protocol: Uniswap router cannot be excluded."

        );

        require(account != address(this), 'State Protocol: The contract it self cannot be excluded');

        require(!_isExcluded[account], "State Protocol: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        ...
        require(_isExcluded[account], "State Protocol: Account is already included");
    ...
    function _transfer(
        ...
        require(now > liquidityAddedAt + 15 minutes  || amount <= 4400e9, "You cannot transfer more than 600 tokens.");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_liquidityFee != 0){
            ...
            uint256 liquidityFee = amount.mul(_liquidityFee).div(10**(_feeDecimal + 8));
