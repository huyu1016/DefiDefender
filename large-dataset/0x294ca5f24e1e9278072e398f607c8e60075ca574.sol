...
// $CHADS Liquidity Pool / 28M tokens up for grabs + 20M for uniswap (chads.vc)

// @dev airblush@protonmail.com (virgin@protonmail.com was taken)

contract ChadsLiquidityPool is Ownable {
    ...
    uint256 public constant INDIVIDUAL_CAP = 1.5 ether;
    ...
    mapping(address => uint256) public chadlists;
    ...
    function addChadlists(address[] calldata accounts) external onlyOwner {

        for (uint256 i = 0; i < accounts.length; i++) {

            chadlists[accounts[i]] = INDIVIDUAL_CAP;
    ...
    function addAndLockLiquidity() external onlyOwner {
        ...
        require(hasEnded(), "ChadsLiquidityPool: can only send liquidity once hardcap is reached");



        uint256 amountEthForUniswap = address(this).balance;

        uint256 amountTokensForUniswap = token.balanceOf(address(this));



        IChadsToken(address(token)).unpause();
        ...
        IChadsToken(address(token)).initializeTwap();
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(beneficiary != address(0), "ChadsLiquidityPool: beneficiary is the zero address");

        require(isOpen(), "ChadsLiquidityPool: sale did not start yet.");

        require(!hasEnded(), "ChadsLiquidityPool: sale is over.");

        require(weiAmount >= MIN_CONTRIBUTION, "ChadsLiquidityPool: weiAmount is smaller than min contribution.");

        require(!isWithinCappedSaleWindow() || (contributions[beneficiary].add(weiAmount) <= chadlists[beneficiary]),

            "ChadsLiquidityPool: individual cap exceeded or not whitelisted"
    ...
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
        ...
        return weiAmount.mul(tokensPerEth);
