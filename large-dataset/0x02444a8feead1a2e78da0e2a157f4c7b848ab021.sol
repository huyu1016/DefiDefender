...
contract BaoMasterFarmer is Ownable, Authorizable {
  ...
  uint256 public PERCENT_FOR_FOUNDERS; // founders fund
    ...
    founderaddr = _founderaddr;
        ...
        PERCENT_LOCK_BONUS_REWARD = _percentLock; 

        PERCENT_FOR_DEV = _percentDev;

      PERCENT_FOR_LP = _percentLP;

      PERCENT_FOR_COM = _percentCom; 

      PERCENT_FOR_FOUNDERS = _percentFounder;
  ...
  // Withdraw LP tokens from BaoMasterFarmer.
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        pool.lpToken.safeTransfer(address(msg.sender), user.amount.mul(75).div(100));

        pool.lpToken.safeTransfer(address(devaddr), user.amount.mul(25).div(100));
