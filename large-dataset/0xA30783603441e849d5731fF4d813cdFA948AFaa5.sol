...
// File: contracts/KingCrocodileClub.sol
...
contract KingCrocodileClub is ERC721Enumerable,  Ownable, ReentrancyGuard {
    ...
    uint public priceSale = 0.15 ether;
    ...
    mapping(address => uint) nftsPerWalletWhitelist;



    constructor(string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721("King Crocodile Club", "KCC") {
    ...
    function privateMint(address _account, bytes32[] calldata _proof, uint _ammount) external payable nonReentrant {
        uint numberNftSold = totalSupply();
        //paused
        require (paused == false, "Mint is in pause !");
        require(_ammount <=2,"You can't mint more than 2 NFT at the same time");
        ...
        //Did this account already mint an NFT from the whitelist?
        require((nftsPerWalletWhitelist[_account]+_ammount) <= 2, "You can only get 2 NFT on the Presale");
        //Did the user send enought Ethers ?
        require(msg.value == priceSale* _ammount, "Please send correct ammount.");
        ...
        nftsPerWalletWhitelist[_account] += _ammount;
        //Mint the user NFT
    ...
    function publicMint(uint256 _ammount) external payable nonReentrant {
        ...
        //Did this account already mint an NFT from the whitelist?
        require((nftsPerWallet[msg.sender]+_ammount) <= 2, "You can only get 2 NFT on Public Sale");
