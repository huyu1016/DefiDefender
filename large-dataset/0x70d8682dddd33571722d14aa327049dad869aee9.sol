...
contract SaffronPool is ISaffronPool {
  ...
  constructor(address _strategy, address _base_asset, address _SFI_address) {
    ...
    SFI_ratio = 1000;          // v1: constant ratio
  ...
  function add_liquidity(uint256 amount, Tranche tranche) external override {
    ...
    require(epoch == 1, "v1: must be epoch 1 only"); // v1: can't add liquidity after epoch 0
  ...
  function sweep(address _to) public {
    ...
    require(block.timestamp > epoch_cycle.start_date + 12 weeks, "v1: must be completed");
