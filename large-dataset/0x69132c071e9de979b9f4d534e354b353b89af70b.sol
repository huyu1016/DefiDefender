...
// File: contracts/interfaces/ICoreFlashArb.sol
...
// File: contracts/jobs/CoreFlashArbRelay3rOptimized.sol
...
contract CoreFlashArbRelay3rNew is Ownable{
    ...
    function getRewardToken(ICoreFlashArb.Strategy memory strat) public view returns (address) {

        // Eg. Token 0 was out so profit token is token 1

        return strat.token0Out[0] ? IUniswapV2Pair(strat.pairs[0]).token1() : IUniswapV2Pair(strat.pairs[0]).token0();
    ...
    function workBatch(uint[] memory profitable) public upkeep{
        ...
        for(uint i=0;i<profitable.length;i++){
            ...
            sendERC20(getRewardToken(CoreArb.strategyInfo(i)),msg.sender);
