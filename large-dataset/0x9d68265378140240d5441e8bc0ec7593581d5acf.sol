...
contract TheStartup is ERC721, PaymentSplitter, Ownable {
    ...
    address private teamWallet = 0x1E73E8563De667bB0C2ac38d754A8cfB18F0AC18;
    ...
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("Sweet Patch Kids", "SPK") PaymentSplitter(_payees, _shares) {
        _mintCards(teamWallet, 25);
    }
    ...
    function mint(uint _amount) external payable isNotPaused(mintEnabled) {
        require((_amount > 0 && _amount < 15), "Wrong amount");
        require(totalSupply() + _amount < 10_001, 'Exceeds max supply');
        require(price * _amount == msg.value, "Wrong ETH amount");
